    ...
    public class HuodongCachingMgr
    {
        ...
        public static int ProcessGetNewStepGift(KPlayer client, int step)
        {
            ...
            if (tongQian > 0)
            {
                ...
                //给用户加钱,更新用户的铜钱
                GameManager.ClientMgr.AddMoney(Global._TCPManager.MySocketListener, Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool, client, tongQian, "新手见面礼物", false);
                GameManager.SystemServerEvents.AddEvent(string.Format("从新手见面奖品领取金币, roleID={0}({1}), Money={2}, newMoney={3}", client.RoleID, client.RoleName, client.Money, tongQian), EventLevels.Record);
            }
            ...
            {
                //给用户加钱,更新用户的绑定元宝
                GameManager.ClientMgr.AddUserBoundMoney(Global._TCPManager.MySocketListener, Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool, client, bindYuanBao, "新手见面礼物");
                GameManager.SystemServerEvents.AddEvent(string.Format("从新手见面奖品领取绑定元宝, roleID={0}({1}), Money={2}, newMoney={3}", client.RoleID, client.RoleName, client.Token, bindYuanBao), EventLevels.Record);
            }
            //设置领取标志
