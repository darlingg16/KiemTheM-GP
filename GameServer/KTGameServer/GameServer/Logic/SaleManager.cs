...
namespace GameServer.Logic
{
    ...
    public class SaleManager : IManager
    {
        ...
        public static void InitConfig()
        {
            ...
            foreach (var kv in GameManager.JiaoYiType.SystemXmlItemDict)
            {
                int id = kv.Key;
                int type = kv.Value.GetIntValue("Type");

                Dictionary<int, List<SaleGoodsData>> item;
                if (!_SaleGoodsDict2.TryGetValue(type, out item))
                {
                    item = new Dictionary<int, List<SaleGoodsData>>();
                    _SaleGoodsDict2.Add(type, item);
                    _TypeHashSet.Add(type);
                }
                ...
                if (null == goodsIDs)
                {
                    ...
                    int occupation = kv.Value.GetIntValue("Occupation");
                    if (null != categoriys)
                    {
                        foreach (var category in categoriys)
                        {
                            _Categoriy2TabIDDict.Add(((long)occupation * 100000000) + category, new int[2] { type, id });
                            //_Categoriy2TabIDDict.Add(((long)occupation << 32) + category, new int[2] { type, id });
                        }
                    }
                    else
                    {
                        OthersGoodsTypeAndID = new int[2] { type, id };
                    }
                }
                else
                {
                    ...
                    {
                        _GoodsID2TabIDDict.Add(goodsID, new int[2] { type, id });
                    }
                }
            }
        }
