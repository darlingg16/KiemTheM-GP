    ...
    public class TCPClient
    {
        ...
        public void Connect(string ip, int port, string serverName)
        {
            ...
            lock (MutexSocket)
            {
                ...
                {
                    //通知主窗口显示连接数
                    RootWindow.AddDBConnectInfo(ListIndex, string.Format("{0}, 与{1}: {2}:{3}建立连接失败", ListIndex, ServerName, ip, port));

                    LogManager.WriteLog(LogTypes.Error, string.Format("{0}, 与{1}: {2}:{3}建立连接失败", ListIndex, ServerName, ip, port));
                    _Socket = null;
                    throw; //继续抛出异常
                }

                Global.SendGameServerHeart(this);   // 心跳
                RootWindow.AddDBConnectInfo(ListIndex, string.Format("{0}, 与{1}: {2}建立连接成功", ListIndex, ServerName, remoteEndPoint));
            }
        }
        ...
        public void Disconnect()
        {
            lock (MutexSocket)
            {
                ...
                RootWindow.AddDBConnectInfo(ListIndex, string.Format("{0}, 与{1}: {2}断开连接", ListIndex, ServerName, Global.GetSocketRemoteEndPoint(_Socket)));
        ...
        public byte[] SendData(TCPOutPacket tcpOutPacket)
        {
            lock (MutexSocket)
            {
                ...
                try
                {
                    ...
                    if (n != 4) //返回失败
                    {
                        LogManager.WriteLog(LogTypes.Error, string.Format("{0}, 与{1}: {2}通讯失败, 获取数据包长度失败", ListIndex, ServerName, Global.GetSocketRemoteEndPoint(_Socket)));
                        return null;
                    }
                    ...
                    {
                        LogManager.WriteLog(LogTypes.Error, string.Format("{0}, 与{1}: {2}通讯失败, 返回的数据包数据长度:{3}和接收到的数据长度:{4}不匹配", ListIndex, ServerName, Global.GetSocketRemoteEndPoint(_Socket), length, totalReceived));
                        return null;
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        string cmdData = new UTF8Encoding().GetString(tcpOutPacket.GetPacketBytes(), 6, tcpOutPacket.PacketDataSize - 6);
                        LogManager.WriteLog(LogTypes.Error, string.Format("{0}, 和{1}:{2}通讯失败, 发送命令{3}, 数据 {4}, 长度{5}, 异常信息:{6}", ListIndex, ServerName, Global.GetSocketRemoteEndPoint(_Socket), (TCPGameServerCmds)tcpOutPacket.PacketCmdID, cmdData, tcpOutPacket.PacketDataSize - 6, ex.Message));

                        //打印错误堆栈
                        DataHelper.WriteExceptionLogEx(ex, string.Format("和{0}通讯发生异常", ServerName));
                    }
                    catch (Exception)
                    {
                    }

                    //LogManager.WriteLog(LogTypes.Error, string.Format("{0}, 与{1}: {2}通讯失败", ListIndex, ServerName, Global.GetSocketRemoteEndPoint(_Socket)));

                    //通知主窗口显示连接数

                    RootWindow.AddDBConnectInfo(ListIndex, string.Format("{0}, 与{1}: {2}通讯失败", ListIndex, ServerName, Global.GetSocketRemoteEndPoint(_Socket)));
                }
