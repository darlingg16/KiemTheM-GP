...
namespace GameServer.Server
{
    ...
    public class TCPManager
    {
        ...
        public void ForceCloseSocket(TMSKSocket s)
        {
            ...
            //TÌM GAME CLIENT
            KPlayer gameClient = GameManager.ClientMgr.FindClient(s);
            if (null != gameClient)
            {
                // CALL TỚI LOGIC LOGOUT CLIENT
                KTLogic.Logout(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, gameClient);
...
//#if false 
                    if (UseWorkerPool)
                    {
                        ...
                        //    KPlayer client = GameManager.ClientMgr.FindClient(tcpInPacket.CurrentSocket);
                        //    if (null != client)
                        //    {
