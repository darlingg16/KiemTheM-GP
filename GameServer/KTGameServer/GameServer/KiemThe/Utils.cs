...
namespace GameServer.KiemThe
{
    ...
    public static class Utils
    {
        ...
        public static bool SendTelegram(string NOIDUNG)
        {
            try
            {
                string URL = "https://api.telegram.org/bot7152601446:AAEp5v8Yr495JKiYcjFa3h0A-MD0W20IOeI/sendMessage?chat_id=-4275401655&text=" + UrlEncode(NOIDUNG);
        ...
        public static bool CheckValidString(string inputString, bool allowWhiteSpaces = false)
        {
            if (!allowWhiteSpaces)
            {
                bool ret = Regex.Matches(inputString, @"\w+")?.Count == 1;
                ret &= !Regex.IsMatch(inputString, @"[^\w]");
                return ret;
            }
            else
            {
