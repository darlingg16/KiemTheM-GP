...
namespace GameServer.KiemThe.GameEvents.TeamBattle
{
    ...
    public static class TeamBattle_ActivityScript
    {
        ...
        public static void MoveToBattleHall(KPlayer player)
        {
            ...
            GameManager.ClientMgr.NotifyChangeMap(TCPManager.getInstance().MySocketListener, TCPOutPacketPool.getInstance(), player, TeamBattle.Config.EnterMapID, TeamBattle.Config.EnterPosX, TeamBattle.Config.EnterPosY, 0, true);
        }
        ...
        public static void ProcessEnterBattle(KPlayer teamPlayer)
        {
            ...
            foreach (KPlayer teammate in teammates)
            {
                ...
                GameManager.ClientMgr.ProcessRoleExperience(teammate, TeamBattle.Award.EnterRound.Exp, true, false, true, "TeamBattle_EnterRoundAward");
                /// Thêm bạc khóa tương ứng
                GameManager.ClientMgr.AddUserBoundMoney(teammate, TeamBattle.Award.EnterRound.BoundMoney, "TeamBattle_EnterRoundAward");
                /// Thêm danh vọng
                KTGlobal.AddRepute(teammate, 701, TeamBattle.Award.EnterRound.Repute);
            }
        }
        ...
        public static void ProcessBattleResult(KPlayer winnerTeamPlayer, int stage, bool increaseStageForWinnerTeam)
        {
            ...
            {
                /// Thêm kinh nghiệm tương ứng
                GameManager.ClientMgr.ProcessRoleExperience(teammate, TeamBattle.Award.WinRound.Exp, true, false, true, "TeamBattle_WinRoundAward");
                /// Thêm bạc khóa tương ứng
                GameManager.ClientMgr.AddUserBoundMoney(teammate, TeamBattle.Award.WinRound.BoundMoney, "TeamBattle_WinRoundAward");
                /// Thêm danh vọng
                KTGlobal.AddRepute(teammate, 701, TeamBattle.Award.WinRound.Repute);
            }
            ...
            if (TeamBattle_ActivityScript.IsRegisteredForBattle(player))
            {
                ...
                foreach (KPlayer teammate in teammates)
                {
                    ...
                    if (teammate != player)
                    {
                        ...
                        PlayerManager.ShowNotification(teammate, msg);
                    }
        ...
        public static string GetAwards(KPlayer player)
        {
            ...
            {
                /// Thêm kinh nghiệm tương ứng
                GameManager.ClientMgr.ProcessRoleExperience(teammate, awardInfo.Exp, true, false, true, "TeamBattle_FinalAward");
                /// Thêm danh vọng tương ứng
                KTGlobal.AddRepute(teammate, 701, awardInfo.Repute);

                /// Duyệt danh sách phần thưởng
                foreach (TeamBattle.AwardInfo.ItemInfo itemInfo in awardInfo.Items)
                {
                    /// Thêm phần thưởng tương ứng
                    ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, teammate, itemInfo.ID, itemInfo.Quantity, 0, "TeamBattle_FinalAward", true, 0, false, Global.ConstGoodsEndTime);
                }
            }
