...
namespace GameServer.KiemThe.GameEvents.TeamBattle
{
    ...
    public class TeamBattle_Script_Main : CopySceneEvent
    {
        ...
        /// </summary>
        private readonly ConcurrentDictionary<int, List<KPlayer>> TeamPlayers = new ConcurrentDictionary<int, List<KPlayer>>();
        ...
        protected override void OnTick()
        {
            ...
            if (this.step == 0)
            {
                ...
                else
                {
                    ...
                    foreach (KeyValuePair<int, List<KPlayer>> pair in this.TeamPlayers)
                    {
                        /// Chọn thành viên đầu tiên trong nhóm
                        KPlayer player = pair.Value.FirstOrDefault();
            ...
            else if (this.step == 1)
            {
                ...
                if (!this.isBattleFinished && this.IsFinished(out KPlayer winnerTeamPlayer))
                {
                    ...
                    PlayerManager.ShowNotificationToAllTeammates(winnerTeamPlayer, "Chúc mừng chiến đội đã giành thắng lợi trong trận đấu lần này!");
        ...
        protected override void OnClose()
        {
            ...
            if (!this.isBattleFinished)
            {
                ...
                if (this.TeamPlayers.TryGetValue(winnerTeamID, out List<KPlayer> winnerTeam))
                {
                    winnerPlayer = winnerTeam.FirstOrDefault();
                }
		...
		public override void OnPlayerEnter(KPlayer player)
        {
            ...
            this.OpenEventBroadboard(player, TeamBattle.Config.EventID);
        ...
        public override void OnPlayerLeave(KPlayer player, GameMap toMap)
        {
            ...
            this.CloseEventBroadboard(player, TeamBattle.Config.EventID);
            /// Chuyển trạng thái PK hòa bình
            player.PKMode = (int) PKMode.Peace;
            /// Chuyển Camp về -1
            player.Camp = -1;
            ...
            if (!this.TeamPlayers.ContainsKey(player.TeamID))
            {
                ...
                {
                    /// Nếu tồn tại
                    if (this.TeamPlayers.TryGetValue(key, out List<KPlayer> playerList))
                    {
            ...
            this.TeamPlayers[player.TeamID].Remove(player);
        ...
        public override void OnPlayerDie(GameObject killer, KPlayer player)
        {
            ...
            PlayerManager.Relive(player, TeamBattle.Config.EnterMapID, TeamBattle.Config.EnterPosX, TeamBattle.Config.EnterPosY, this.CopyScene.ReliveHPPercent, this.CopyScene.ReliveMPPercent, this.CopyScene.ReliveStaminaPercent);
        }
        #endregion
        ...
        private void TickPlayersTeamInfo()
        {
            foreach (KPlayer player in this.GetPlayers())
            {
                ...
                {
                    this.TeamPlayers[player.TeamID] = new List<KPlayer>();
                }
                /// Nếu chưa tồn tại
                if (!this.TeamPlayers[player.TeamID].Contains(player))
                {
        ...
        private void KickOutAllPlayers()
        {
            foreach (KPlayer player in this.GetPlayers())
            {
                GameManager.ClientMgr.NotifyChangeMap(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, player, TeamBattle.Config.EnterMapID, TeamBattle.Config.EnterPosX, TeamBattle.Config.EnterPosY, (int) player.CurrentDir);
            }
        }
        #endregion
    }
