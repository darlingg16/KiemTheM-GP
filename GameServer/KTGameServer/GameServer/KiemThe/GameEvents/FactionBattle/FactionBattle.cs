...
namespace GameServer.KiemThe.GameEvents.FactionBattle
{
    public class FactionBattle
    {
        ...
        public void Revice(KPlayer Player)
        {
            ...
            {
                KT_TCPHandler.GetPlayerCopySceneInfoFromDB(Player, out int copySceneID, out int preMapCode, out int prePosX, out int prePosY, out long copySceneCreateTicks);

                GameManager.ClientMgr.NotifyChangeMap(TCPManager.getInstance().MySocketListener, TCPOutPacketPool.getInstance(), Player, preMapCode, prePosX, prePosY, 0, false);
            }
            ...
            else if (BATTLESTATE == FactionState.MELEE_ROUND_1 || BATTLESTATE == FactionState.MELEE_ROUND_2)
            {
                ...
                PlayerManager.Relive(Player, GetMapCode, _GetPostion.X, _GetPostion.Y, 100, 100, 100);
            }
            ...
            else /*if(BATTLESTATE == FactionState.ELIMINATION_ROUND_1 || BATTLESTATE == FactionState.ELIMINATION_ROUND_2 || BATTLESTATE == FactionState.ELIMINATION_ROUND_3 || BATTLESTATE == FactionState.ELIMINATION_ROUND_4)*/
            {
                ...
                PlayerManager.Relive(Player, GetMapCode, _Config.GoIn.X, _Config.GoIn.Y, 100, 100, 100);
            }
        }
        ...
        public void MovePlayerToArena(KPlayer KPlayer)
        {
            ...
            if (BATTLESTATE == FactionState.SIGN_UP || BATTLESTATE == FactionState.FREE_MELEE || BATTLESTATE == FactionState.READY_ELIMINATION || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_1 || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_2 || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_3 || BATTLESTATE == FactionState.CHAMPION_AWARD || BATTLESTATE == FactionState.ELIMINATION_ROUND_1 || BATTLESTATE == FactionState.ELIMINATION_ROUND_2 || BATTLESTATE == FactionState.ELIMINATION_ROUND_3 || BATTLESTATE == FactionState.ELIMINATION_ROUND_4)
            {
                ...
                KT_TCPHandler.RecordCopySceneInfoToDB(KPlayer, this.GetMapCode, KPlayer.CurrentMapCode, (int)KPlayer.CurrentPos.X, (int)KPlayer.CurrentPos.Y, -1);
                // Chuyển người chơi vào map
                GameManager.ClientMgr.NotifyChangeMap(TCPManager.getInstance().MySocketListener, TCPOutPacketPool.getInstance(), KPlayer, this.GetMapCode, _Config.GoIn.X, _Config.GoIn.Y, 0, false);
            ...
            else if (BATTLESTATE == FactionState.MELEE_ROUND_1 || BATTLESTATE == FactionState.MELEE_ROUND_2)
            {
                ...
                GameManager.ClientMgr.NotifyChangeMap(TCPManager.getInstance().MySocketListener, TCPOutPacketPool.getInstance(), KPlayer, this.GetMapCode, _Position.X, _Position.Y, 0, false);
        ...
        public int Register(KPlayer player)
        {
            ...
            //if ((int)BATTLESTATE > (int)FactionState.SIGN_UP)
            //{
            //    return -2;
            //} //Bạn không phải là đệ tử của môn phái này，không thể tham gia thi đấu!
        ...
        public int MaxBoxCanEarn(FactionPlayer _OutPlayer, int Rank)
        {
            if (Rank == 16)
            {
                return 2;
        ...
        public void NpcQuanQuan(GameMap map, NPC npc, KPlayer client)
        {
            ...
            if (_OutPlayer != null)
            {
                if (BATTLESTATE == FactionState.CHAMPION_AWARD)
                {
                    ...
                    Text += "Điểm hiện tại của bạn là: <color=#00ff2a>" + _OutPlayer.nScore + "</color>, Có thể đổi: <color=#00ff2a>" + GetMaxExpCanEarn(_OutPlayer, RANKING) + "</color> kinh nghiệm, <color=#00ff2a>" + MaxBoxCanEarn(_OutPlayer, RANKING) + "</color> bảo rương.\n";
        ...
        private void DoActionSelect(FactionPlayer outPlayer, NPC npc, TaskCallBack Select, int RANKING)
        {
            if (Select.SelectID == -3)
            {
                ...
                if (outPlayer.player.RoleID == _FINDQUANQUAN.WinThisRound.player.RoleID)
                {
                    ...
                    {
                        outPlayer.player.AddRoleTitle(GetTitleIDByFactionID(this.FactionID));
                        ...
                        for (int i = 0; i < Random; i++)
                        {
                            ...
                            GameMap Map = GameManager.MapMgr.DictMaps[this.GetMapCode];

                            // TẠON BOX Ở VỊ TRÍ TƯƠNG ỨNG
                            CreateBox(Map, _Postion.X, _Postion.Y);
                        }
                    }
                    else
                    {
            ...
            else if (Select.SelectID == -4)
            {
                ...
                {
                    int BOX = MaxBoxCanEarn(outPlayer, RANKING);
                    if (BOX > 0)
                    {
                        if (!KTGlobal.IsHaveSpace(BOX, outPlayer.player))
                        {
                            KTLuaLib_Player.SendMSG("Không đủ chỗ trống để nhận thưởng", outPlayer.player, npc);
                            return;
                        }
                        else
                        {
                            ...
                            if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, outPlayer.player, 335, BOX, 0, "TDMP", false, 1, false, Global.ConstGoodsEndTime))
                            {
                                PlayerManager.ShowNotification(outPlayer.player, "Có lỗi khi nhận vật phẩm chế tạo");
                            }

                            int CAMP = 300 + outPlayer.player.m_cPlayerFaction.GetFactionId();

                            KTGlobal.AddRepute(outPlayer.player, CAMP, 10);

                            long MaxExp = GetMaxExpCanEarn(outPlayer, RANKING);

                            if (MaxExp > 0)
                            {
                                GameManager.ClientMgr.ProcessRoleExperience(outPlayer.player, MaxExp, true, false, true, "BATTLE_FACTIONBATTLE");
                            }
                ...
                else if (BATTLESTATE == FactionState.MELEE_ROUND_2) // Nếu như đang trong trận PVP tự do
                {
                    ...
                    if (TimeUtil.NOW() >= LastTick + (_Config.TimeAct.MELEE_PVP_DULATION * 1000) && BATTLESTATE == FactionState.MELEE_ROUND_2)
                    {
                        ...
                        else
                        {
                            ...
                            KTGlobal.SendFactionChat(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, this.FactionID, "Không đủ số người tham gia đăng ký.TDMP sẽ không thể bắt đầu");
                ...
                else if (BATTLESTATE == FactionState.READY_ELIMINATION || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_1 || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_2 || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_3)
                {
                    ...
                    if (TimeUtil.NOW() - LastCreateFlag >= 60 * 1000 && (BATTLESTATE == FactionState.READY_ELIMINATION || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_1 || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_2 || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_3))
                    {
                        ...
                        for (int i = 0; i < 20; i++)
                        {
                            ...
                            CreateFLAG(Map, _PointFlag.X, _PointFlag.Y);
                        }
                    }
                    ...
                    if (TimeUtil.NOW() >= LastTick + (_Config.TimeAct.FREE_ELIMINATION * 1000) && (BATTLESTATE == FactionState.READY_ELIMINATION || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_1 || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_2 || BATTLESTATE == FactionState.FREE_ELIMINATION_ROUND_3))
                    {
                        ...
                        foreach (ELIMINATION_INFO ARENA in ELIMINATION_TOTAL_ROUND)
                        {
                            ...
                            if (ARENA.Player_1.player != null && ARENA.Player_1.player.IsOnline() && ARENA.Player_1.player.MapCode == this.GetMapCode)
                            {
                                ...
                                PlayerManager.ShowNotification(ARENA.Player_1.player, "Thời gian chuẩn bị cho trận chiến là 60s");
                                ...
                                {
                                    GameManager.ClientMgr.NotifyOthersGoBack(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, ARENA.Player_1.player, _ARENAPOSTION.PosX, _ARENAPOSTION.PosY, (int)ARENA.Player_1.player.CurrentDir);
                                    // DỊch thằng này vào bản đồ
                                    // GameManager.ClientMgr.NotifyChangeMap(TCPManager.getInstance().MySocketListener, TCPOutPacketPool.getInstance(), ARENA.Player_1.player, this.GetMapCode, _ARENAPOSTION.PosX, _ARENAPOSTION.PosY, 0, false);
                                }
                            }
                            ...
                            if (ARENA.Player_2.player != null && ARENA.Player_2.player.IsOnline() && ARENA.Player_2.player.MapCode == this.GetMapCode)
                            {
                                ...
                                //Send thông báo cho người chơi biết là trận đấu đã bắt đầu
                                PlayerManager.ShowNotification(ARENA.Player_2.player, "Thời gian chuẩn bị cho trận chiến là 60s");
                                // Gửi thông báo về cho thằng này thông tin trận đấu
                                ...
                                if (_ARENAPOSTION != null)
                                {
                                    GameManager.ClientMgr.NotifyOthersGoBack(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, ARENA.Player_2.player, _ARENAPOSTION.PosX, _ARENAPOSTION.PosY, (int)ARENA.Player_2.player.CurrentDir);
                                }
                            }
                ...
                else if (BATTLESTATE == FactionState.ELIMINATION_ROUND_1 || BATTLESTATE == FactionState.ELIMINATION_ROUND_2 || BATTLESTATE == FactionState.ELIMINATION_ROUND_3 || BATTLESTATE == FactionState.ELIMINATION_ROUND_4)
                {
                    ...
                    if (TimeUtil.NOW() - LastTick >= 60 * 1000 && _ROUNDSTATE == ROUNDSTATE.PREADING && (BATTLESTATE == FactionState.ELIMINATION_ROUND_1 || BATTLESTATE == FactionState.ELIMINATION_ROUND_2 || BATTLESTATE == FactionState.ELIMINATION_ROUND_3 || BATTLESTATE == FactionState.ELIMINATION_ROUND_4))
                    {
                        ...
                        foreach (ELIMINATION_INFO ARENA in ELIMINATION_TOTAL_ROUND)
                        {
                            ...
                            //Send thông báo cho người chơi biết là trận đấu đã bắt đầu
                            PlayerManager.ShowNotification(ARENA.Player_1.player, "PVP đã chính thức bắt đầu!");
                            ...
                            //Send thông báo cho người chơi biết là trận đấu đã bắt đầu
                            PlayerManager.ShowNotification(ARENA.Player_2.player, "PVP đã chính thức bắt đầu!");
                    ...
                    if (TimeUtil.NOW() >= LastTick + (_Config.TimeAct.ELIMINATION_ROUND_ROUND * 1000) && (BATTLESTATE == FactionState.ELIMINATION_ROUND_1 || BATTLESTATE == FactionState.ELIMINATION_ROUND_2 || BATTLESTATE == FactionState.ELIMINATION_ROUND_3 || BATTLESTATE == FactionState.ELIMINATION_ROUND_4))
                    {
                        ...
                        List<ELIMINATION_INFO> ELIMINATION_TOTAL_ROUND = ELIMINATION_TOTAL.Where(x => x.ROUNDID == ROUNID && x.Player_1.IsWinRound == false && x.Player_2.IsWinRound == false).ToList();
                        ...
                        foreach (ELIMINATION_INFO ARENA in ELIMINATION_TOTAL_ROUND)
                        {
                            ...
                            {
                                PlayerManager.ShowNotification(ARENA.Player_1.player, "Trong trận tỉ với " + ARENA.Player_2.player.RoleName + " bạn đã gây ra tổng " + KTGlobal.CreateStringByColor(DAMGEPLAYER1 + "", ColorType.Importal) + " sát thương dành chiến thắng");

                                PlayerManager.ShowNotification(ARENA.Player_2.player, "Trong trận tỉ với " + ARENA.Player_1.player.RoleName + " bạn đã thua vì gây ra ít sát thương hơn");
                            ...
                            {
                                PlayerManager.ShowNotification(ARENA.Player_2.player, "Trong trận tỉ với " + ARENA.Player_1.player.RoleName + " bạn đã gây ra tổng " + KTGlobal.CreateStringByColor(DAMGEPLAYER2 + "", ColorType.Importal) + " sát thương dành chiến thắng");

                                PlayerManager.ShowNotification(ARENA.Player_1.player, "Trong trận tỉ với " + ARENA.Player_2.player.RoleName + " bạn đã thua vì gây ra ít sát thương hơn");
                        ...
                        foreach (ELIMINATION_INFO ARENA in ELIMINATION_TOTAL_ROUND)
                        {
                            ...
                            ARENA.Player_1.player.ForbidUsingSkill = true;
                            //Send thông báo cho người chơi biết là trận đấu đã bắt đầu
                            PlayerManager.ShowNotification(ARENA.Player_1.player, "Trận chiến đã kết thúc");

                            //GameManager.ClientMgr.NotifyChangeMap(TCPManager.getInstance().MySocketListener, TCPOutPacketPool.getInstance(), ARENA.Player_1.player, this.GetMapCode, _Config.GoIn.X, _Config.GoIn.Y, 0, false);

                            GameManager.ClientMgr.NotifyOthersGoBack(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, ARENA.Player_1.player, _Config.GoIn.X, _Config.GoIn.Y, (int)ARENA.Player_1.player.CurrentDir);
                            ...
                            ARENA.Player_2.player.ForbidUsingSkill = true;
                            //Send thông báo cho người chơi biết là trận đấu đã bắt đầu
                            PlayerManager.ShowNotification(ARENA.Player_2.player, "Trận chiến đã kết thúc");

                            // GameManager.ClientMgr.NotifyChangeMap(TCPManager.getInstance().MySocketListener, TCPOutPacketPool.getInstance(), ARENA.Player_2.player, this.GetMapCode, _Config.GoIn.X, _Config.GoIn.Y, 0, false);

                            GameManager.ClientMgr.NotifyOthersGoBack(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, ARENA.Player_2.player, _Config.GoIn.X, _Config.GoIn.Y, (int)ARENA.Player_2.player.CurrentDir);
                        ...
                        else
                        {
                            ...
                            if (_FINDQUANQUAN != null)
                            {
                                ...
                                PlayerManager.Relive(_FINDQUANQUAN.Player_1.player, GetMapCode, _Config.GoIn.X, _Config.GoIn.Y, 100, 100, 100);
                                PlayerManager.Relive(_FINDQUANQUAN.Player_2.player, GetMapCode, _Config.GoIn.X, _Config.GoIn.Y, 100, 100, 100);
        ...
        public void ClickFlag(KPlayer client, int ResID)
        {
            ...
            if (ResID == 2701)
            {
                if (client.TeamID != -1)
                {
                    ...
                    foreach (KPlayer member in TotalMember)
                    {
                        ...
                        if (!Global.InCircle(new Point(member.PosX, member.PosY), client.CurrentPos, 2000))
                        {
                            continue;
                        }
            ...
            else if (ResID == 2700)
            {
                ...
                if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, client, 335, 1, 0, "TDMP", false, 1, false, Global.ConstGoodsEndTime))
                {
            ...
            GrowPoint growPoint = new GrowPoint()
            {
                ...
                ConditionCheck = (player) =>
                {
                    ...
                    if (_PLAYCOLLECT != null)
                    {
                        ...
                        {
                            PlayerManager.ShowNotification(player, "Bạn đã thu thập đủ cờ không thể thu thập thêm");
                            return false;
                        }
                    }
        ...
        public void ExpProsecc()
        {
            foreach (KeyValuePair<int, FactionPlayer> entry in PVPARENA)
            {
                ...
                {
                    GameManager.ClientMgr.ProcessRoleExperience(PlayerBattle.player, 16000, true, false, true, "TDMP");
                }
            }
        }
        ...
        /// </summary>
        ///
        public void SendPvpToOtherPlayer(FactionPlayer InputPlayer, int Sec, int STATUS)
        {
        ...
        public void AntiLagMove(Point CenterPoint, FactionPlayer Character)
        {
            ...
            {
                GameManager.ClientMgr.NotifyOthersGoBack(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, Character.player, (int)CenterPoint.X, (int)CenterPoint.Y, (int)Character.player.CurrentDir);
            }
        }
        ...
        public void UpdatePVP1VS1TOOtherPlayer(int ROUNID)
        {
            ...
            foreach (ELIMINATION_INFO ARENAINFO in ELIMINATION_TOTAL_ROUND)
            {
                ...
                if (_ROUNDSTATE == ROUNDSTATE.START)
                {
                    ...
                    if (ARENAINFO._ROUNDSTATE == ROUNDSTATE.PREADING)
                    {
            ...
            foreach (KeyValuePair<int, FactionPlayer> entry in PVPARENA)
            {
                ...
                if (PlayerBattle.player.CurrentMapCode == this.GetMapCode)
                {
                    ...
                    //SendPvpToOtherPlayer(PlayerBattle, FinalSec);
                    ...
                    if (_ROUNDSTATE == ROUNDSTATE.PREADING)
                    {
                        ...
                        SendPvpToOtherPlayer(PlayerBattle, FinalSec, 1);
                    }
                    ...
                    if (_ROUNDSTATE == ROUNDSTATE.START)
                    {
                        ...
                        SendPvpToOtherPlayer(PlayerBattle, FinalSec, 2);
                    }
                }
            }
        }
        ...
        public void NotifyALLTOBATTLE(string MSG)
        {
            foreach (KeyValuePair<int, FactionPlayer> entry in PVPARENA)
            {
                ...
                PlayerManager.ShowNotification(PlayerBattle.player, MSG);
            }
        }
        ...
        public void SendBattleNotify1VS1(FactionPlayer InputPlayer, ROUNDSTATE _STATE, int Sec, ELIMINATION_INFO _INFO)
        {
            ...
            _Notify.TotalInfo.Add(KTGlobal.CreateStringByColor(_INFO.Player_1.player.RoleName, ColorType.Yellow));

            _Notify.TotalInfo.Add(KTGlobal.CreateStringByColor("====VS====", ColorType.Importal));

            _Notify.TotalInfo.Add(KTGlobal.CreateStringByColor(_INFO.Player_2.player.RoleName, ColorType.Yellow));
        ...
        public void MoveAllPlayerToPvp()
        {
            ...
            foreach (KeyValuePair<int, FactionPlayer> entry in PVPARENA)
            {
                ...
                if (PlayerBattle.player.CurrentMapCode == GetMapCode)
                {
                    ...
                    GameManager.ClientMgr.NotifyOthersGoBack(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, PlayerBattle.player, _Position.X, _Position.Y, (int)PlayerBattle.player.CurrentDir);

                    // Gửi thông báo về thời gian đăng ký còn bao lâu cho bọn nó biết
                    // PlayerManager.Relive(PlayerBattle.player, this.GetMapCode, _Position.X, _Position.Y, 100, 100, 100);
                }
            }
        ...
        public void MoveAllPlayerOutPvp()
        {
            ...
            foreach (KeyValuePair<int, FactionPlayer> entry in PVPARENA)
            {
                ...
                if (PlayerBattle.player.CurrentMapCode == GetMapCode)
                {
                    ...
                    //    var Action = new Action(() => PlayerManager.Relive(PlayerBattle.player, this.GetMapCode, _Config.GoIn.X, _Config.GoIn.Y, 100, 100, 100));

                    //    var MoveValue = this.MoveDelayTask(Action, 2000);
                    //}

                    // Gửi thông báo về thời gian đăng ký còn bao lâu cho bọn nó biết
                    PlayerManager.Relive(PlayerBattle.player, this.GetMapCode, _Config.GoIn.X, _Config.GoIn.Y, 100, 100, 100);
                }
            }
                            ...
                            if (_ARENAFIND.Player_1.player.RoleID == LOGOUT.RoleID || _ARENAFIND.Player_2.player.RoleID == LOGOUT.RoleID)
                            {
                                if (_ARENAFIND.Player_1.player.RoleID == LOGOUT.RoleID)
                                {
                                    ...
                                    if (_ARENAFIND.Player_2 != null)
                                    {
                                        ...
                                        Action _Action = new Action(() => GameManager.ClientMgr.NotifyOthersGoBack(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, _ARENAFIND.Player_2.player, _Config.GoIn.X, _Config.GoIn.Y, (int)_ARENAFIND.Player_2.player.CurrentDir));
                                    ...
                                    PlayerManager.ShowNotification(_ARENAFIND.Player_2.player, "Trong trận tỉ với " + _ARENAFIND.Player_1.player.RoleName + " bạn đã dành chiến thắng");

                                    PlayerManager.ShowNotification(_ARENAFIND.Player_1.player, "Trong trận tỉ với " + _ARENAFIND.Player_2.player.RoleName + " bạn đã thua cuộc");
                                }
                                else if (_ARENAFIND.Player_2.player.RoleID == LOGOUT.RoleID)
                                {
                                    ...
                                    if (_ARENAFIND.Player_1 != null)
                                    {
                                        ...
                                        Action _Action = new Action(() => GameManager.ClientMgr.NotifyOthersGoBack(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, _ARENAFIND.Player_1.player, _Config.GoIn.X, _Config.GoIn.Y, (int)_ARENAFIND.Player_1.player.CurrentDir));
                                    ...
                                    PlayerManager.ShowNotification(_ARENAFIND.Player_1.player, "Trong trận tỉ với " + _ARENAFIND.Player_2.player.RoleName + " bạn đã dành chiến thắng");

                                    PlayerManager.ShowNotification(_ARENAFIND.Player_2.player, "Trong trận tỉ với " + _ARENAFIND.Player_1.player.RoleName + " bạn đã thua cuộc");
                                }
                            }
                        }
                    }
