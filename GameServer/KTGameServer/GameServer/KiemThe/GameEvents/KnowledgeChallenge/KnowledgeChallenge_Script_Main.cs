...
namespace GameServer.KiemThe.GameEvents.KnowledgeChallenge
{
    ...
    public class KnowledgeChallenge_Script_Main : GameMapEvent
    {
        ...
        {
            int ret = player.GetValueOfDailyRecore((int) AcitvityRecore.KnowledgeChallenge_TotalQuestions);
            if (ret < 0)
            {
                ret = 0;
            }
        ...
        {
            player.SetValueOfDailyRecore((int) AcitvityRecore.KnowledgeChallenge_TotalQuestions, count);
        }
        ...
        private void CreateIdeNPC(UnityEngine.Vector2Int pos)
        {
            ...
            this.RemoveNPC();
            /// Tạo NPC mới
            NPCGeneralManager.AddNewNPC(this.Map.MapCode, -1, KnowledgeChallenge.Event.NPC.NPCID, pos.x, pos.y, KnowledgeChallenge.Event.NPC.Name, KnowledgeChallenge.Event.NPC.Title, Entities.Direction.DOWN, -1, "KnowledgeChallenge", (npc) => {
                /// Lưu lại thông tin NPC vừa tạo
                this.IdeNPC = npc;
                npc.MinimapName = "Sứ giả Hoa Đăng";
        ...
        private void CreateMovingNPC(UnityEngine.Vector2Int toPos)
        {
            ...
            this.RemoveNPC();
            /// Tạo NPC mới
            GameManager.MonsterZoneMgr.AddDynamicMonsters(this.Map.MapCode, KnowledgeChallenge.Event.NPC.MonsterID, -1, 1, oldPos.x, oldPos.y, KnowledgeChallenge.Event.NPC.Name, KnowledgeChallenge.Event.NPC.Title, 100, 80, Entities.Direction.DOWN, Entities.KE_SERIES_TYPE.series_none, Entities.MonsterAIType.DynamicNPC, -1, -1, -1, "KnowledgeChallenge", (npc) => {
                /// Lưu lại thông tin NPC vừa tạo
                this.MovingNPC = npc;
                /// Miễn dịch toàn bộ
        ...
        public void NPCClick(NPC npc, KPlayer player)
        {
            ...
            {
                /// Thông báo
                PlayerManager.ShowNotification(player, "Không tìm thấy NPC tương ứng!");
                /// Bỏ qua
                return;
            }
            /// Nếu sự kiện đã kết thúc
            ...
            {
                /// Thông báo
                PlayerManager.ShowNotification(player, "Sự kiện Đoán hoa đăng lần này đã kết thúc, bằng hữu hãy quay lại sau!");
                /// Bỏ qua
                return;
            }
            /// Nếu cấp độ không đủ
            ...
            {
                /// Thông báo
                PlayerManager.ShowNotification(player, string.Format("Bằng hữu cần đạt tối thiểu cấp {0} mới có thể tham gia sự kiện Đoán hoa đăng!", KnowledgeChallenge.Config.LimitLevel));
                /// Bỏ qua
                return;
            }
            ...
            else if (KTGlobal.GetCurrentTimeMilis() - player.LastOpenKnowledgeChallengeQuestion < KnowledgeChallenge.Event.Config.DelayTicksEachQuestion)
            {
                ...
                /// Thông báo
                PlayerManager.ShowNotification(player, string.Format("Bằng hữu vừa mở câu hỏi, cần chờ {0} giây nữa mới có thể mở câu hỏi mới!", delaySec));
                /// Bỏ qua
                return;
            }
            ...
            try
            {
                ...
                {
                    /// Thông báo
                    PlayerManager.ShowNotification(player, string.Format("Bằng hữu đã tham gia trả lời tổng cộng {0}/{1} câu hỏi, không thể tham gia thêm!", totalQuestions, KnowledgeChallenge.Event.Config.MaxQuestions));
                    /// Bỏ qua
                    return;
                }
                ...
                dialog.OnSelect = (data) => {
                    ...
                    {
                        /// Thông báo
                        PlayerManager.ShowNotification(player, "Thật đáng tiếc, câu trả lời của bằng hữu không chính xác, hãy thử lại lần sau!");
                        /// Đóng Dialog
                        KT_TCPHandler.CloseDialog(player);
                        /// Xóa khỏi danh sách quản lý để tránh BUG Click nhiều lần
                        KTNPCDialogManager.RemoveNPCDialog(dialog);
                    ...
                    {
                        /// Thông báo
                        PlayerManager.ShowNotification(player, "Chúc mừng bằng hữu đã trả lời đúng!");
                        /// Đóng Dialog
                        KT_TCPHandler.CloseDialog(player);
                        /// Xóa khỏi danh sách quản lý để tránh BUG Click nhiều lần
                        KTNPCDialogManager.RemoveNPCDialog(dialog);
                        /// Thưởng bạc khóa
                        GameManager.ClientMgr.AddUserBoundMoney(player, KnowledgeChallenge.Award.BoundMoney, "KnowledgeChallenge");
                        /// Thưởng đồng khóa
                        GameManager.ClientMgr.AddBoundToken(Global._TCPManager.MySocketListener, Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool, player, KnowledgeChallenge.Award.BoundToken, "KnowledgeChallenge");
                        /// Thưởng kinh nghiệm
                        GameManager.ClientMgr.ProcessRoleExperience(player, KnowledgeChallenge.Award.Exp, true, false, true, "KnowledgeChallenge");
                        /// Thưởng uy danh
