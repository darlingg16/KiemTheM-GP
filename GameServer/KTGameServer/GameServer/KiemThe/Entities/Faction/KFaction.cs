...
namespace GameServer.KiemThe.Entities
{
    ...
    public static class KFaction
    {
        ...
        public class KRoute
        {
            ...
            /// <summary>
            /// Phải phái ngoại không
            /// </summary>
            public bool isPhysical { get; set; }

            /// <summary>
            /// Có sử dụng tốc độ đánh không
            /// </summary>
        ...
        public static void Init()
        {
            ...
            XElement xmlNode = Global.GetGameResXml("Config/KT_Faction/Faction.xml");
            foreach (XElement node in xmlNode.Elements("Faction"))
            {
                KFactionAttirbute factionAttirbute = new KFactionAttirbute()
                {
                    ...
                    nSeries = (KE_SERIES_TYPE)int.Parse(node.Attribute("Elemental").Value),
                ...
                foreach (XElement subNode in node.Elements("Sub"))
                {
                    KRoute route = new KRoute()
                    {
                        ...
                        useAS = bool.Parse(subNode.Attribute("UseAS").Value),
                        isPhysical = int.Parse(subNode.Attribute("IsPhysical").Value) == 1,
                    };
        ...
        /// <summary>
        /// Kiểm tra nhánh phái tương ứng có ảnh hưởng bởi tốc đánh không
        /// </summary>
        /// <param name="byFactionId"></param>
        /// <param name="byRouteId"></param>
        /// <returns></returns>
        public static bool UseAS(int byFactionId, int byRouteId)
        {
            KFactionAttirbute faction = KFaction.m_sAttribute.Where(x => x.nID == byFactionId).FirstOrDefault();
            if (faction == null)
            {
        ...
        /// <param name="byRouteId"></param>
        /// <returns></returns>
        public static bool IsPhysical(int byFactionId, int byRouteId)
        {
            KFactionAttirbute faction = KFaction.m_sAttribute.Where(x => x.nID == byFactionId).FirstOrDefault();
            if (faction == null)
            {
