...
namespace GameServer.KiemThe.Core.Activity.PlayerPray
{
	...
	public static class KTPlayerPrayManager
	{
		...
		public static void Init()
		{
			...
			XElement xmlNode = Global.GetGameResXml("Config/KT_Activity/PlayerPray.xml");
		...
		public static int GetTotalTurnLeft(KPlayer player)
		{
			...
			int totalTurnLeft = player.GetValueOfDailyRecore((int) AcitvityRecore.PlayerPray);
			/// Nếu sang ngày mới
			if (totalTurnLeft == -1)
			{
		...
		public static void SetTotalTurnLeft(KPlayer player, int value)
		{
			...
			player.SetValueOfDailyRecore((int) AcitvityRecore.PlayerPray, value);
		}
		...
		public static bool Open(KPlayer player, out List<string> lastResult, out bool enableGetAward, out bool enableStartTurn)
		{
			...
			{
				PlayerManager.ShowNotification(player, string.Format("Cần đạt cấp {0} trở lên mới có thể Chúc phúc!", KTPlayerPrayManager.MinLevel));
				return false;
			}
		...
		public static bool StartTurn(KPlayer player, out int round, out int stopPos, out bool enableGetAward, out bool enableStartTurn)
		{
			...
			{
				PlayerManager.ShowNotification(player, "Hãy nhận phần thưởng trước!");
				return false;
			}
			/// Nếu đã quá số lượt
			else if (player.LastPrayResult != null && player.LastPrayResult.Count >= 5)
			{
				PlayerManager.ShowNotification(player, "Đã quá số lượt có thể quay!");
				return false;
			}
			...
			if (player.LastPrayResult == null || player.LastPrayResult.Count <= 0)
			{
				...
				if (totalTurnLeft <= 0)
				{
					PlayerManager.ShowNotification(player, "Bạn đã hết lượt Chúc phúc trong ngày, hãy quay lại vào ngày mai!");
					return false;
				}
		...
		public static bool GetAward(KPlayer player)
		{
			...
			{
				PlayerManager.ShowNotification(player, "Không có quà thưởng!");
				return false;
			}
			...
			{
				PlayerManager.ShowNotification(player, string.Format("Cần sắp xếp lại tối thiểu {0} khoảng trống trong túi để nhận thưởng!", spaceNeed));
				return false;
			}
			...
			foreach (KeyValuePair<int, int> pair in playerPrayData.Items)
			{
				...
				if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, player, pair.Key, pair.Value, 0, "PlayerPray", true, 1, false, Global.ConstGoodsEndTime))
				{
			...
			PlayerManager.ShowNotification(player, string.Format("Nhận thưởng chúc phúc thành công, danh vọng chúc phúc tăng {0} điểm!", playerPrayData.Repute));
