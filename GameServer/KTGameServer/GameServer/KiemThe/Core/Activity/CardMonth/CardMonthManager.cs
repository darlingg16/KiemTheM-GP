...
namespace GameServer.KiemThe.Core.Activity.CardMonth
{
    public class CardMonthManager
    {
        ...
        {
            string Files = Global.GameResPath(YUE_KA_GOODS_FILE);
            ...
            if (null != client)
            {
                lock (client.YKDetail)
                {
                    if (client.YKDetail.HasYueKa == 0)
                    {
                        ...
                        client.YKDetail.BegOffsetDay = Global.GetOffsetDay(nowDate);
                        client.YKDetail.EndOffsetDay = client.YKDetail.BegOffsetDay + DAYS_PER_YUE_KA;
                        client.YKDetail.CurOffsetDay = Global.GetOffsetDay(nowDate);
                    ...
                    GameManager.ClientMgr.NotifySelfParamsValueChange(client, RoleCommonUseIntParamsIndexs.YueKaRemainDay, client.YKDetail.RemainDayOfYueKa());
        ...
        public static TCPProcessCmdResults ActiveCardMoth(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, global::Server.Protocol.TCPOutPacketPool pool, int nID, byte[] data, int count, out global::Server.Protocol.TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                PlayerManager.ShowMessageBox(client, "Chú ý ", "Bạn có muốn mua thẻ tháng này với giá :" + YUE_KA_MONEY_ID_IN_CHARGE_FILE + " Đồng?", new Action(() => SureBuy(client)),true);
        ...
        private static void SureBuy(KPlayer client)
        {
            ...
            {
                PlayerManager.ShowNotification(client, "Đồng không đủ");
                return;
            }

            SubRep _REP = KTGlobal.SubMoney(client, YUE_KA_MONEY_ID_IN_CHARGE_FILE, Entities.MoneyType.Dong, "CARDMONTH");

            if (_REP.IsOK)
            {
                HandleUserBuyYueKa(client.strUserID, client.RoleID);
                PlayerManager.ShowNotification(client, "Kích hoạt thẻ tháng thành công");
                ...
                GameManager.ClientMgr.SendToClient(client, DataHelper.ObjectToBytes<YueKaData>(ykData), (int)TCPGameServerCmds.CMD_SPR_GET_YUEKA_DATA);
                return;
            }
            else
            {
        ...
        private static YueKaError _GetYueKaAward(KPlayer client, int day)
        {
            ...
            lock (client.YKDetail)
            {
                ...
                if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, client, ItemID, ItemNumer, 0, "CARDMONTH", false, 1, false, Global.ConstGoodsEndTime,"",-1,"",0,1,true))
                {
        ...
        public static void UpdateNewDay(KPlayer client)
        {
            ...
            lock (client.YKDetail)
            {
                // Notify dữ liệu thẻ tháng về client
                GameManager.ClientMgr.NotifySelfParamsValueChange(client, RoleCommonUseIntParamsIndexs.YueKaRemainDay, client.YKDetail.RemainDayOfYueKa());
            }
        }
    }
