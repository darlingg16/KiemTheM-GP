...
namespace GameServer.KiemThe.Core.Activity.SeashellCircle
{
    ...
    public static class KTSeashellCircleManager
    {
        ...
        public static void Init()
        {
            ...
            XElement xmlNode = Global.GetGameResXml("Config/KT_Activity/KTSeashellCircle.xml");
        ...
        public static bool GetSeashell(KPlayer player, out int totalSeashells)
        {
            ...
            {
                PlayerManager.ShowNotification(player, "Xin lỗi, bạn không có quà để nhận!");
                return false;
            }
            ...
            if (KTSeashellCircleManager.CanExchange(player))
            {
                ...
                if (lastStage < 0 || lastStage >= KTSeashellCircleManager.Data.ExchangeSeashells.Count)
                {
                    PlayerManager.ShowNotification(player, "Bạn không có phần thưởng có thể quy đổi qua vỏ sò!");
                    return false;
                }
                ...
                {
                    PlayerManager.ShowNotification(player, string.Format("Bạn cần sắp xếp {0} ô trống trong túi đồ để nhận vỏ sò!", totalSpacesNeed));
                    return false;
                }
                ...
                if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, player, KTSeashellCircleManager.Seashells, shellsCount, 0, "SeashellCircle", true, 0, false, Global.ConstGoodsEndTime))
                {
                    PlayerManager.ShowNotification(player, "Có lỗi khi tạo vật phẩm, hãy liên hệ với hỗ trợ để được trợ giúp!");
                ...
                PlayerManager.ShowNotification(player, string.Format("Đổi thành công, bạn nhận được {0} vỏ sò!", shellsCount));
        ...
        public static bool GetAward(KPlayer player, out int awardType, out int itemID, out int itemNumber)
        {
            ...
            if (KTSeashellCircleManager.HasTresureToGet(player))
            {
                ...
                {
                    PlayerManager.ShowNotification(player, string.Format("Bạn cần sắp xếp {0} ô trống trong túi đồ để nhận quà!", awardRate));
                    return false;
                }
                ...
                /// Tạo vật phẩm cho người chơi
                if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, player, KTSeashellCircleManager.Treasure, awardRate, 0, "SeashellCircle", true, 0, false, Global.ConstGoodsEndTime))
                {
                    PlayerManager.ShowNotification(player, "Có lỗi khi tạo vật phẩm, hãy liên hệ với hỗ trợ để được trợ giúp!");
                ...
                /// Thông báo đổi thành công
                PlayerManager.ShowNotification(player, string.Format("Nhận thành công, bạn nhận được {0} cái [{1}]!", awardRate, KTGlobal.GetItemName(KTSeashellCircleManager.Treasure)));
            ...
            else if (KTSeashellCircleManager.CanGet(player))
            {
                ...
                switch (lastCellType)
                {
                    ...
                    case (int)KTSeashellCircle.SeashellCircleCellType.CrystalStone:
                        {
                            /// Tạo vật phẩm cho người chơi
                            if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, player, value, awardRate, 0, "SeashellCircle", true, 0, false, Global.ConstGoodsEndTime))
                            {
                                PlayerManager.ShowNotification(player, "Có lỗi khi tạo vật phẩm, hãy liên hệ với hỗ trợ để được trợ giúp!");
                            ...
                            /// Thông báo đổi thành công
                            PlayerManager.ShowNotification(player, string.Format("Nhận thành công, bạn nhận được {0} cái [{1}]!", awardRate, KTGlobal.GetItemName(value)));
                    ...
                    case (int)KTSeashellCircle.SeashellCircleCellType.GatherMakePoint:
                        {
                            ...
                            /// Thông báo đổi thành công
                            PlayerManager.ShowNotification(player, string.Format("Nhận thành công!"));
                    ...
                    case (int)KTSeashellCircle.SeashellCircleCellType.Money:
                        {
                            GameManager.ClientMgr.AddMoney(player, value * awardRate, "SeashellCircle");
                    ...
                    case (int)KTSeashellCircle.SeashellCircleCellType.BoundToken:
                        {
                            GameManager.ClientMgr.AddBoundToken(Global._TCPManager.MySocketListener, Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool, player, value * awardRate, "SeashellCircle");
        ...
        public static bool StartTurn(KPlayer player, int bet, out int lastStopPos, out int durationTick, out int totalCells, out bool isFirstTurn)
        {
            ...
            if (KTSeashellCircleManager.HasTresureToGet(player))
            {
                PlayerManager.ShowNotification(player, "Bạn có phần quà cần nhận trước mới có thể quay tiếp!");
                return false;
            }
            ...
            {
                PlayerManager.ShowNotification(player, "Bạn hiện không thể thực hiện quay, hãy liên hệ với hỗ trợ để được xử lý!");
                return false;
            }
            ...
            {
                PlayerManager.ShowNotification(player, "Thao tác bị lỗi. Số sò cược giữa 2 lần liên tiếp không khớp!");
                return false;
            }
            ...
            if (isFirstTurn)
            {
                ...
                {
                    PlayerManager.ShowNotification(player, "Số sò mang theo không đủ!");
                    return false;
                }
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        PlayerManager.ShowNotification(player, "Dữ liệu bị lỗi, hãy liên hệ với hỗ trợ để được trợ giúp!");
            ...
            if (isFirstTurn)
            {
                ...
                {
                    PlayerManager.ShowNotification(player, "Không thể xóa số sò đã cược, xin hãy thử lại!");
