...
namespace GameServer.KiemThe.Core.Item
{
    public class ItemRefine
    {
        ...
        {
            string Files = Global.GameResPath(ConfigURL);
        ...
        public static int DoRefine(KPlayer client, GoodsData InputData, GoodsData Suff, List<GoodsData> TotalCrytals, int ItemDescSelect)
        {
            ...
            if (_SourceFind == null)
            {
                PlayerManager.ShowNotification(client, "Vật phẩm luyện hóa không hợp lệ");
                return -1;
            }
            ...
            if (SuffItem == null)
            {
                PlayerManager.ShowNotification(client, "Chế tạo đồ đặt vào không hợp lệ");
                return -2;
            }
            ...
            if (DestItem == null)
            {
                PlayerManager.ShowNotification(client, "Vật phẩm muốn nhận không hợp lệ");
                return -20;
            }
            ...
            if (find == null)
            {
                PlayerManager.ShowNotification(client, "Không tìm thấy công thức này");
                return -3;
            }
            ...
            if (TotalCrytals != null)
            {
                if (TotalCrytals.Count > 0)
                {
                    foreach (GoodsData FixItem in TotalCrytals)
                    {
                        ...
                        {
                            PlayerManager.ShowNotification(client, "Vật phẩm đặt vào không đúng");
                            return -4;
                        }
                    }
                }
            ...
            if (Percent < 100)
            {
                PlayerManager.ShowNotification(client, "Tỉ lệ phải đạt 100% mới có thể luyện hóa vui lòng bỏ thêm huyền tinh");
                return -4;
            }
            ...
            {
                PlayerManager.ShowNotification(client, "Cần tối thiểu 1 chỗ trống trong túi đồ mới có thể luyện hóa");
                return -6;
            }

            if (nMoney < 0)
            {
                PlayerManager.ShowNotification(client, "Có lỗi trong công thức tính toán");
                return -8;
            }
            ...
            if (MoneyLess < 0)
            {
                ...
                if (!SubMoney.IsOK)
                {
                    PlayerManager.ShowNotification(client, "Không thể trừ bạc khóa tiền");
                    return -6;
                }
                ...
                if (!SubMoney.IsOK)
                {
                    PlayerManager.ShowNotification(client, "Không thể trừ bạc thương");
                    return -6;
                }
            }
            else
            ...
            {
                PlayerManager.ShowNotification(client, "Không thể xóa trang bị nguồn");
                return -7;
            }

            if (!ItemManager.AbandonItem(Suff, client, false, "REFINE_" + InputData.Id))
            {
                PlayerManager.ShowNotification(client, "Không thể xóa luyện hóa đồ");
                return -8;
            }
            ...
            if (TotalCrytals != null)
            {
                if (TotalCrytals.Count > 0)
                {
                    foreach (GoodsData FixItem in TotalCrytals)
                    {
                        ...
                        {
                            PlayerManager.ShowNotification(client, "Không thể huyền tinh");
                            return -9;
                        }
                    }
                }
            ...
            if (ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, client, ItemDescSelect, 1, 0, "COMPOSECRYTAL", false, 1, false, Global.ConstGoodsEndTime, "", -1, "", InputData.Forge_level, 0, true))
            {
                PlayerManager.ShowNotification(client, "Luyện hóa thành công :" + DestItem.Name);
            ...
            {
                PlayerManager.ShowNotification(client, "Có lỗi khi thêm huyền tinh vào túi đồ vui lòng liên hệ ADMIN để được giúp đỡ");
