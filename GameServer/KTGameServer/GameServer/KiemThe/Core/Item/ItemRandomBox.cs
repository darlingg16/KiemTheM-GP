...
namespace GameServer.KiemThe.Core.Item
{
    public class ItemRandomBox
    {
        ...
        {
            string Files = Global.GameResPath(RandomBoxConfig);
        ...
        public static bool OpenBoxRandom(KPlayer player, int boxDbID)
        {
            ...
            {
                PlayerManager.ShowNotification(player, "Cần ít nhất 6 ô trống để mở");
                return false;
            }

            /// Vật phẩm tương ứng
            GoodsData itemGD = Global.GetGoodsByDbID(player, boxDbID);
            /// Nếu toác
            if (itemGD == null)
            {
                PlayerManager.ShowNotification(player, "Vật phẩm không khả dụng!");
                return false;
            }
            /// Thông tin vật phẩm
            ...
            if (findbox != null)
            {
                ...
                if (LimitWeek > 0)
                {
                    ...
                    {
                        PlayerManager.ShowNotification(player, "Tuần này bạn đã mở hết số lượt cho phép");
                        return false;
                    }
                }
                ...
                if (LimitDay > 0)
                {
                    ...
                    {
                        PlayerManager.ShowNotification(player, "Hôm nay bạn đã mở hết số lượt cho phép");
                        return false;
                    }
                }
                ...
                if (_SelectItem != null)
                {
                    ...
                    {
                        GameManager.ClientMgr.ProcessRoleExperience(player, _SelectItem.Number, true, false, true, "RANDOMBOX_" + boxID);
                        return true;
                    }

                    if (_SelectItem.Type == 0)
                    {
                        string TimeFix = Global.ConstGoodsEndTime;
                    ...
                    if (_SelectItem.Type == 6)
                    {
                        /// Nếu không có vật phẩm
                        if (player.GoodsDataList == null)
                        {
                            PlayerManager.ShowNotification(player, "Bạn chưa trang bị mật tịch.");
                            return false;
                        }
                        ...
                        {
                            book = player.GoodsDataList.Where(x => x.Using == (int) KE_EQUIP_POSITION.emEQUIPPOS_BOOK).FirstOrDefault();
                        }
                        /// Nếu không có sách
                        if (book == null)
                        {
                        ...
                        MonsterDeadHelper.BookProseccExp(player, _SelectItem.Number, false);
                        return true;
                    }
                }
                else
