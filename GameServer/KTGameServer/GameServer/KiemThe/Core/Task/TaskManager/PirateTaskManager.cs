...
namespace GameServer.KiemThe.Core.Task
{
    ...
    public class PirateTaskManager
    {
        ...
        {
            int Count = client.GetValueOfDailyRecore((int)AcitvityRecore.PirateTaskNumber);
        ...
        {
            client.SetValueOfDailyRecore((int)AcitvityRecore.PirateTaskNumber, Value);
        }
        ...
        {
            int Count = client.GetValueOfDailyRecore((int)AcitvityRecore.PirateTaskID);
        ...
        {
            client.SetValueOfDailyRecore((int)AcitvityRecore.PirateTaskID, Value);
        }
            ...
            else
            {
                ...
                else
                {
                    if (ItemManager.RemoveItemFromBag(client, 489, 300,-1,"Đổi phần thưởng hải tặc"))
                    {
                        if (Number == 100)
                        {
                            if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, client, 490, 1, 0, "SPLITITEM", false, 1, false, Global.ConstGoodsEndTime))
                            {
                        ...
                        {
                            if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, client, 492, 1, 0, "SPLITITEM", false, 1, false, Global.ConstGoodsEndTime))
                            {
                                PlayerManager.ShowNotification(client, "Có lỗi khi nhận vật phẩm chế tạo");
                            }
                        }
