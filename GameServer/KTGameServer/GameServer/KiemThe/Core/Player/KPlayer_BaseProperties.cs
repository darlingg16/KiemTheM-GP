...
namespace GameServer.Logic
{
	...
	public partial class KPlayer
	{
		...
		public int m_wBonusPoint { get; set; }
		...
		public bool CheckAssignPotential(ref int nStrengthAssign, ref int nDexterityAssign, ref int nVitalityAssign, ref int nEnergyAssign)
		{
            ...
            {
				PlayerManager.ShowNotification(this, "Có lỗi trong quá trình phân phối điểm tiềm năng. Tất cả điểm tiềm năng bị làm mới từ đầu.");
        ...
        {
			/// ID kỹ năng
			SkillLevelRef Skill = this.Skills.GetSkillLevelRef(this.LastMainSkillID);
			
			/// Tốc đánh
			int nAttackSpeed;
			/// Nếu là môn phái hệ ngoại
			if(this.m_cPlayerFaction.IsPhysical())
            {
				nAttackSpeed = this.GetCurrentAttackSpeed();
			}
			/// Nếu là môn phái hệ nội
			else
            {
				nAttackSpeed = this.GetCurrentCastSpeed();
			}				

			RoleAttributes roleData = new RoleAttributes();

			roleData.AtkSpeed = nAttackSpeed;
			roleData.Crit = this.m_CurrentDeadlyStrike;
			...
			/// Nếu toác
			if (Skill == null || Skill.Level <= 0 || Skill.Properties == null)
			{
				roleData.Damage = 0;
			}
            else
            {
				try
                {
					roleData.Damage = AlgorithmProperty.GetDamageInfo(this, Skill, 0);
				}
				catch (Exception ex)
                {
					/// Ghi Log
			...
			roleData.OtherProperties[(int) MAGIC_ATTRIB.magic_poisontimeenhance_p] = this.m_nPoisonTimeEnhanceP;
		...
		public void ChangeCurStrength(int nValueChange)
		{
			...
			int nDamagePhysicsChange = KPlayerSetting.GetStrength2DamagePhysics(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), nCurStrengthNew) -
									   KPlayerSetting.GetStrength2DamagePhysics(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), this.m_wCurStrength);
		...
		public void ChangeCurDexterity(int nValueChange)
		{
			...
			int nAttackRateChange = KPlayerSetting.GetDexterity2AttackRate(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), nCurDexterityNew) -
									KPlayerSetting.GetDexterity2AttackRate(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), this.m_wCurDexterity);
			this.ChangeAttackRating(nAttackRateChange, 0, 0);

			int nDefenceChange = KPlayerSetting.GetDexterity2Defence(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), nCurDexterityNew) -
								 KPlayerSetting.GetDexterity2Defence(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), this.m_wCurDexterity);
			this.ChangeDefend(nDefenceChange, 0, 0);

			int nDamagePhysicsChange = KPlayerSetting.GetDexterity2DamagePhysics(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), nCurDexterityNew) -
									   KPlayerSetting.GetDexterity2DamagePhysics(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), this.m_wCurDexterity);
			this.ChangePhysicsDamage(nDamagePhysicsChange);
		...
		public void ChangeCurVitality(int nValueChange)
		{
			...
			int nLifeChange = KPlayerSetting.GetVitality2Life(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), nCurVitalityNew) -
							  KPlayerSetting.GetVitality2Life(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), this.m_wCurVitality);
		...
		public void ChangeCurEnergy(int nValueChange)
		{
			...
			int nManaChange = KPlayerSetting.GetEnergy2Mana(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), nCurEnergyNew) -
							  KPlayerSetting.GetEnergy2Mana(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), this.m_wCurEnergy);
			this.ChangeManaMax(nManaChange, 0, 0);

			int nDamageMagicChange = KPlayerSetting.GetEnergy2DamageMagic(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), nCurEnergyNew) -
									 KPlayerSetting.GetEnergy2DamageMagic(this.m_cPlayerFaction.GetFactionId(), this.m_cPlayerFaction.GetRouteId(), this.m_wCurEnergy);
