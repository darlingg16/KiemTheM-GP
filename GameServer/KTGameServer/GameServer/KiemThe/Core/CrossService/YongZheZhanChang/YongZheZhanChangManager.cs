    ...
    public partial class YongZheZhanChangManager : IManager, ICmdProcessorEx, IEventListener, IEventListenerEx, IManager2
    {
        ...
        private void TimerProc(object sender, EventArgs e)
        {
            ...
            if (null != list)
            {
                ...
                {
                    KPlayer client = GameManager.ClientMgr.FindClient(kuaFuServerLoginData.RoleId);
                    if (null != client)
                    {
        ...
        // 检查勇者战场当前处于什么时间状态
        private int CheckCondition(KPlayer client, ref YongZheZhanChangSceneInfo sceneItem, ref YongZheZhanChangGameStates state)
        {
            int result = 0;
            sceneItem = null;
