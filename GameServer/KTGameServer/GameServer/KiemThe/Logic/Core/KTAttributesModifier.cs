...
namespace GameServer.KiemThe.Logic
{
	...
	public static class KTAttributesModifier
	{
		...
		/// <param name="magic"></param>
		/// <param name="nSkillId"></param>
		private static void ChangeCampV(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			/*if (!obj.IsPlayer())
			{
				if (magic.nValue[0] > 0 && magic.nValue[0] < (int)NPCCAMP.camp_num)
				{
					obj.m_CurrentCamp = magic.nValue[0];
				}
				else
				{
					obj.m_CurrentCamp = obj.Camp;
				}
			}*/
		...
		private static void DexterityV(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			...
			((KPlayer) obj).ChangeCurDexterity(magic.nValue[0] * stackCount);
		}
		...
		private static void EnergyV(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			...
			((KPlayer) obj).ChangeCurEnergy(magic.nValue[0] * stackCount);
		}
		...
		/// <param name="magic"></param>
		/// <param name="nSkillId"></param>
		private static void SlowMissileB(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			if (magic.nValue[0] > 0)
			{
				obj.m_CurrentSlowMissile = true;
			}
			else
			{
				obj.m_CurrentSlowMissile = false;
			}
		}
		...
		private static void StealLifeP(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			...
			obj.m_nStealLifeRate += magic.nValue[1] * stackCount;
		}
		...
		private static void StealManaP(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			...
			obj.m_nStealManaRate += magic.nValue[1] * stackCount;
		}
		...
		private static void StealStaminaP(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			...
			obj.m_nStealStaminaRate += magic.nValue[1] * stackCount;
		}
		...
		private static void StrengthV(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			...
			((KPlayer) obj).ChangeCurStrength(magic.nValue[0] * stackCount);
		}
		...
		private static void VitalityV(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			...
			((KPlayer) obj).ChangeCurVitality(magic.nValue[0] * stackCount);
		}
		...
		{
			obj.AddShieldState(obj.m_CurrentManaMax * magic.nValue[0] * stackCount / 100, magic.nValue[1] * stackCount);
		}
		...
		private static void InfectPoison(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			...
			if (g_cMagicAttribMgr.IsEndProcess())
			{
				_ASSERT(FALSE); // ²»Ó¦¸Ã½«´ËÊôÐÔÌîÔÚ×´Ì¬¼¼ÄÜÖÐ
				return;
			}
			KNpc* pLauncher = g_cMagicAttribMgr.GetMagicLauncher();
			if (!pLauncher)
			{
				_ASSERT(FALSE); // ´ËÊôÐÔÐèÒª·¢ËÍÕß
				return;
			}
			if (obj.HaveState(KNpc::emSTATE_POISON))
				return; // ÒÑÓÐ¶¾×´Ì¬£¬²»ÔÙ´«È¾
			obj.AddPoisonState(pLauncher->m_nLastPoisonDamageIdx,
				pLauncher->m_state[KNpc::emSTATE_POISON].m_nParam * magic.nValue[0] / 100,
				pLauncher->m_state[KNpc::emSTATE_POISON].GetRestFrame());
			*/
		}
		...
		private static void AddSkillLevel(GameObject obj, KMagicAttrib magic, int stackCount, int nSkillId)
		{
			...
			if (cMagic.nValue[0] == 0)
			{
				/*obj.m_SkillList.m_cSkillAddition.AddAllSkillAddtion(cMagic);*/
			}
			else
			{
				/*obj.m_SkillList.m_cSkillAddition.AddMagicAddtion(cMagic.nValue[0], cMagic, g_cMagicAttribMgr.IsEndProcess());*/
		...
		public static void AttachProperty(KMagicAttrib magicAttrib, GameObject go, bool isDetachPhrase, int stackCount = 1, int skillID = -1, int skillLevel = 1)
		{
			...
			switch (magicAttrib.nAttribType)
			{
				...
				case MAGIC_ATTRIB.magic_slowmissile_b:
					KTAttributesModifier.SlowMissileB(go, magicAttrib, stackCount, skillID);
					break;
				case MAGIC_ATTRIB.magic_changecamp_b:
					KTAttributesModifier.ChangeCampV(go, magicAttrib, stackCount, skillID);
					break;
