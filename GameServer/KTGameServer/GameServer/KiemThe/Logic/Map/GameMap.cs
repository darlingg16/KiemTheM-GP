...
namespace GameServer.Logic
{
    ...
    public class GameMap
    {
        ...
        public void InitMap()
        {
            ...
            Console.WriteLine("Begin load map ID = {0} - {1}", this.MapCode, this.MapName);
        ...
        {
            string name = string.Format("MapConfig/{0}/Obs.xml", this.MapConfigDir);
            XElement xml;
            try
            {
                xml = Global.GetResXml(name);
            ...
            string obstructionFileName = Global.GameResPath(string.Format("MapConfig/{0}/Obs.txt", this.MapConfigDir));
            ...
            string blurFileName = Global.GameResPath(string.Format("MapConfig/{0}/Blur.txt", this.MapConfigDir));
        ...
        /// <returns></returns>
        public bool CanMove(Point realPos)
        {
            ...
            if (!this.MyNodeGrid.isWalkable((int) (realPos.X / this.MapGridWidth), (int) (realPos.Y / this.MapGridHeight)))
            {
                return false;
            }
        ...
        public bool CanMove(int gridX, int gridY)
        {
            ...
            if (!this.MyNodeGrid.isWalkable(gridX, gridY))
            {
                return false;
            }
