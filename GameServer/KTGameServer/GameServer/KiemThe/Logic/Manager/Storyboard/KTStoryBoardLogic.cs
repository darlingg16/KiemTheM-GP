...
namespace GameServer.KiemThe.Logic
{
    ...
    public static class KTStoryBoardLogic
    {
        ...
        /// <returns></returns>
        private static bool IsValidPos(int mapCode, UnityEngine.Vector2 pos)
        {
            GameMap gameMap = GameManager.MapMgr.GetGameMap(mapCode);
            return Global.IsGridReachable(mapCode, (int) pos.x / gameMap.MapGridWidth, (int) pos.y / gameMap.MapGridHeight);
        }
        ...
        public static bool StepMove(GameObject obj, ConcurrentQueue<UnityEngine.Vector2> pathsList, float elapseTimeSec, KE_NPC_DOING action)
		{
            ...
            bool DoStepMove(GameObject go, ConcurrentQueue<UnityEngine.Vector2> paths, float elapseTime)
            {
                ...
                {
                    //LogManager.WriteLog(LogTypes.RolePosition, string.Format("Storyboard of {0} has stopped because of trying too much times.", go.RoleName));
                    //Console.WriteLine(string.Format("Storyboard of {0} has stopped because of trying too much times.", go.RoleName));
                ...
                try
                {
					...
					{
                        //LogManager.WriteLog(LogTypes.RolePosition, string.Format("Storyboard of {0} has been stopped due to external force.", go.RoleName));
                        //Console.WriteLine(string.Format("Storyboard of {0} has been stopped due to external force.", go.RoleName));
                    ...
                    if (moveSpeed <= 0)
                    {
                        //LogManager.WriteLog(LogTypes.RolePosition, string.Format("Storyboard of {0} has stopped because movespeed < 0. Stop pos: 1", go.RoleName));
                        //Console.WriteLine(string.Format("Storyboard of {0} has stopped because movespeed < 0. Stop pos: 1", go.RoleName));
                        /// Trả ra kết quả dừng StoryBoard đang thực thi
                        return true;
                    }
                    ...
                    if (paths.Count <= 0)
                    {
                        //LogManager.WriteLog(LogTypes.RolePosition, string.Format("Storyboard of {0} has stopped because move path is empty. Stop pos: 1", go.RoleName));
                        //Console.WriteLine(string.Format("Storyboard of {0} has stopped because move path is empty. Stop pos: 1", go.RoleName));
                        /// Trả ra kết quả dừng StoryBoard đang thực thi
                        return true;
                    }
                    ...
                    if (!KTStoryBoardLogic.IsValidPos(go.CurrentMapCode, nextPos))
                    {
                        //LogManager.WriteLog(LogTypes.RolePosition, string.Format("Storyboard of {0} has stopped because nextPos is not valid. Stop pos: 1", go.RoleName));
                        //Console.WriteLine(string.Format("Storyboard of {0} has stopped because nextPos is not valid. Stop pos: 1", go.RoleName));
                        /// Trả ra kết quả dừng StoryBoard đang thực thi
                        return true;
                    }
