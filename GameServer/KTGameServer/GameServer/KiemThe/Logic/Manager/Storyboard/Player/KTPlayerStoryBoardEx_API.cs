	...
	public partial class KTPlayerStoryBoardEx
	{
		...
		public void Remove(KPlayer player, bool sendToClient = true)
        {
            ...
            try
            {
                ...
                if (this.objectTimers.TryGetValue(player.RoleID, out PlayerStoryBoard objectTimer))
                {
                    ...
                    player.ToPos = player.CurrentPos;

                    /// Đồng bộ về Client
                    if (sendToClient)
                    {
                        GameManager.ClientMgr.NotifyOthersStopMyMoving(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, player);
                    }
                }
            }
            catch (Exception ex)
        ...
        public void Add(KPlayer player, Point fromPos, Point toPos, KE_NPC_DOING action = KE_NPC_DOING.do_run, bool forceMoveBySpecialState = false)
        {
            try
            {
                ...
                GameMap gameMap = GameManager.MapMgr.DictMaps[player.CurrentMapCode];

                /// Thực hiện tìm đường
                List<UnityEngine.Vector2> nodeList = KTGlobal.FindPath(player, new UnityEngine.Vector2((float) fromPos.X, (float) fromPos.Y), new UnityEngine.Vector2((float) toPos.X, (float) toPos.Y));
                ...
                this.SendPlayerMoveToClient(player, pathStr, fromPos, toPos, player.CurrentDir, action);
        ...
        public void Add(KPlayer player, string pathString, KE_NPC_DOING action = KE_NPC_DOING.do_run, bool forceMoveBySpecialState = false)
        {
            try
            {
                ...
                for (int i = 0; i < points.Length; i++)
                {
                    ...
                    if (!GameManager.MapMgr.GetGameMap(player.CurrentMapCode).CanMove(new Point(posX, posY)))
                    {
                        continue;
                    }
