	...
	public partial class KTMonsterStoryBoardEx
	{
        ...
        public void Add(Monster monster, Point fromPos, Point toPos, KE_NPC_DOING action = KE_NPC_DOING.do_run, bool forceMoveBySpecialState = false, bool useAStarPathFinder = false)
        {
            try
            {
                ...
                GameMap gameMap = GameManager.MapMgr.DictMaps[monster.CurrentMapCode];
				...
				{
                    nodeList = KTGlobal.FindPath(monster, new UnityEngine.Vector2((float) fromPos.X, (float) fromPos.Y), new UnityEngine.Vector2((float) toPos.X, (float) toPos.Y));
                }
				else
				{
                ...
                this.SendMonsterMoveToClient(monster, pathStr, fromPos, toPos, monster.CurrentDir, action);
        ...
        public void Add(Monster monster, string pathString, KE_NPC_DOING action = KE_NPC_DOING.do_run, bool forceMoveBySpecialState = false)
        {
            try
            {
                ...
                for (int i = 0; i < points.Length; i++)
                {
                    ...
                    if (!GameManager.MapMgr.GetGameMap(monster.CurrentMapCode).CanMove(new Point(posX, posY)))
                    {
                        continue;
                    }
