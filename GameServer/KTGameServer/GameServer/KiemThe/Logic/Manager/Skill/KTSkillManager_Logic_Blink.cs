...
namespace GameServer.KiemThe.Logic
{
    ...
    public static partial class KTSkillManager
    {
        ...
        private static UnityEngine.Vector2 DoSkillBlinkToPosition(SkillLevelRef skill, GameObject caster, PropertyDictionary skillPd)
        {
            ...
            destPos = KTGlobal.FindLinearNoObsPoint(gameMap, casterPos, destPos);
            ...
            GameManager.MapGridMgr.DictGrids.TryGetValue(caster.CurrentMapCode, out MapGrid mapGrid);
            if (mapGrid != null)
            {
                mapGrid.MoveObject(-1, -1, (int) caster.CurrentPos.X, (int) caster.CurrentPos.Y, caster);
            }
        ...
        private static void DoSkillBlinkTowardTargets(SkillLevelRef skill, GameObject caster, GameObject firstTarget, PropertyDictionary skillPd)
        {
            ...
            List<GameObject> nearByTargets = KTLogic.GetNearByEnemies(caster, range);
            ...
            if (flySkill != null)
            {
                ...
                for (int i = 1; i <= totalPoints; i++)
                {
                    ...
                    KTSkillManager.SetTimeout(time, () => {
                        if (flySkill != null)
                        {
                            KTSkillManager.DoCallChildSkill(skill, flySkillID, caster, tickPos, target, tickDirVector);
                        }
                    });
                }
            }
