...
namespace GameServer.KiemThe.Logic
{
    ...
    public static partial class KTSkillManager
    {
        ...
        private static UnityEngine.Vector2 DoBulletSkillLogic(GameObject caster, UnityEngine.Vector2 castPos, GameObject target, UnityEngine.Vector2? targetPos, UnityEngine.Vector2? dirVectorProperty, SkillLevelRef skill, PropertyDictionary skillPd, bool isChildSkill, int[] initParams)
        {
            ...
            if (skill.Data.Form == 0 || skill.Data.Form == 7 || skill.Data.Form == 8)
            {
                ...
                if (bulletConfig.MoveKind == 1 || bulletConfig.MoveKind == 5)
                {
                    ...
                    if (targetPos.HasValue)
                    {
                        ...
                        _targetPos = KTGlobal.FindLinearNoObsPoint(gameMap, setPos, _targetPos);
            ...
            /// Nếu là kỹ năng thả đạn chuyển động từ từ theo đường tròn với 2 điểm đối xứng nhau
        ...
        private static void DoCallChildSkill(SkillLevelRef skill, int childSkillID, GameObject caster, UnityEngine.Vector2 castPos, GameObject target, UnityEngine.Vector2? dirVectorProperty, int[] initParams = null)
        {
            ...
            if (bulletSkill != null)
            {
                ...
                KTSkillManager.UseSkill_DoAction(caster, castPos, target, dirVectorProperty, skillRef, true, initParams);
            }
        }
        #endregion
        ...
        private static void DoAction(GameObject caster, UnityEngine.Vector2 castPos, GameObject target, UnityEngine.Vector2? targetPos, UnityEngine.Vector2? dirVectorProperty, SkillLevelRef skill, PropertyDictionary skillPd, bool isChildSkill, int[] initParams)
        {
            ...
            if (skill.Data.Type == 5)
            {
                ...
                if (!skill.Data.IsBullet)
                {
                    ...
                    KTSkillManager.DoCallChildSkill(skill, skill.Data.BulletID, caster, childSkillPos, target, dirVectorProperty);
                ...
                else if (skill.Data.Form == 12)
                {
                    ...
                    void Execute()
                    {
                        ...
                        if (!skill.Data.IsBullet)
                        {
                            ...
                            KTSkillManager.DoCallChildSkill(skill, skill.Data.BulletID, caster, skill.Data.StartPoint == 2 && target != null ? currentTargetPos : skill.Data.StartPoint == 3 ? currentCasterPos : childSkillPos, target, dirVectorProperty, new int[] { angle });
                            bulletChildSkillCalled = true;
                        }
                        else
                        {
            ...
            {
                KMagicAttrib magicAttrib = enchantPd.Get<KMagicAttrib>((int) MAGIC_ATTRIB.magic_skilladdition_addstartskill);
            ...
            {
                int subSkillID = skillPd.Get<KMagicAttrib>((int) MAGIC_ATTRIB.magic_skill_startevent).nValue[0];
                int subSkillLevel = skill.Level;

                /// Nếu tồn tại Symbol quy định cấp độ kỹ năng
                if (skillPd.ContainsKey((int) MAGIC_ATTRIB.magic_skill_eventskilllevel))
                {
                    subSkillLevel = skillPd.Get<KMagicAttrib>((int) MAGIC_ATTRIB.magic_skill_eventskilllevel).nValue[0];
                }
