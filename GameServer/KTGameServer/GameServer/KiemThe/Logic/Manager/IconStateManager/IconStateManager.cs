...
namespace GameServer.Logic.RefreshIconState
{
    ...
    public class IconStateManager
    {
        ...
        {
            string Files = Global.GameResPath(IconStateManager.KTEveryDayEvent_XML);
        ...
        private FunctionButtonAction GetIconState(FunctionButtonType IconID, KPlayer client)
        {
            ...
            switch (IconID)
            {
                ...
                {
                    if (ClientManager.CheckEmailCount(client)) // Nếu có email chưa đọc thì sẽ báo
                    {
                        state = FunctionButtonAction.Hint;
                    }
                    break;
        ...
        private int HasWelfareFirstRecharge(KPlayer client)
        {
           int totalChongZhiMoney = GameManager.ClientMgr.QueryTotaoChongZhiMoney(client);
            ...
            int resoult = RechageManager.GetBtnIndexState(totalChongZhiMoney, 1, !(Global.CanGetFirstChongZhiDaLiByUserID(client)));
