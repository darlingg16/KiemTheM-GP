    ...
    public static partial class KT_TCPHandler
    {
            ...
            try
            {
                ...
                foreach (SkillLevelRef skill in client.Skills.ListSkills)
                {
                    skills.Add(new SkillData()
                    {
                        ...
                        BonusLevel = client.Skills.AllSkillBonusLevel,
                ...
                byte[] cmdData = DataHelper.ObjectToBytes<KeyValuePair<int, List<SkillData>>>(new KeyValuePair<int, List<SkillData>>(client.GetCurrentSkillPoints(), skills));
                GameManager.ClientMgr.SendToClient(client, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_RENEW_SKILLLIST);
            }
            catch (Exception ex)
            {
        ...
        public static void AddSkillToDB(KPlayer player, int skillID)
        {
            try
            {
                ...
                GameManager.DBCmdMgr.AddDBCmd((int) TCPGameServerCmds.CMD_DB_ADDSKILL, strcmd, null, player.ServerId);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void DeleteSkillFromDB(KPlayer player, int skillID)
        {
            ...
            {
                string strcmd = string.Format("{0}:{1}", player.RoleID, skillID);
                GameManager.DBCmdMgr.AddDBCmd((int) TCPGameServerCmds.CMD_DB_DEL_SKILL, strcmd, null, player.ServerId);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void UpdateSkillInfoFromDB(KPlayer player, int skillID, int skillLevel, long lastUsedTick, int cooldownTick, int exp)
        {
            ...
            try
            {
                ...
                GameManager.DBCmdMgr.AddDBCmd((int) TCPGameServerCmds.CMD_DB_UPSKILLINFO, strcmd, null, player.ServerId);
        ...
        public static TCPProcessCmdResults ResponseDistributeSkillPoints(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                if (currentAuraSkill != null)
                {
                    ...
                    KTSkillManager.UseSkill(client, client, currentAuraSkill, true);
                }
        ...
        public static TCPProcessCmdResults ResponseSetAndActivateAura(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                //{
                //    PlayerManager.ShowNotification(client, "Bản đồ hiện tại không cho phép sử dụng kỹ năng!");
                //    /// Gói lại gửi Client
                //    byte[] _resultData = DataHelper.ObjectToBytes<G2C_UseSkillResult>(new G2C_UseSkillResult()
                //    {
                //        Result = (int) KTSkillManager.UseSkillResult.None,
                //        PosX = -1,
                //        PosY = -1,
                //    });
                ...
                //}
                ///// Nếu kỹ năng bị cấm
                //else if (gameMap.BanSkills.Contains(skillID))
                //{
                //    PlayerManager.ShowNotification(client, "Bản đồ hiện tại không cho phép sử dụng kỹ năng này!");
                //    /// Gói lại gửi Client
                //    byte[] _resultData = DataHelper.ObjectToBytes<G2C_UseSkillResult>(new G2C_UseSkillResult()
                //    {
                //        Result = (int) KTSkillManager.UseSkillResult.None,
                ...
                //{
                //    PlayerManager.ShowNotification(client, "Đang trong trạng thái bán hàng, không thể sử dụng kỹ năng!");
                //    /// Gói lại gửi Client
                //    byte[] _resultData = DataHelper.ObjectToBytes<G2C_UseSkillResult>(new G2C_UseSkillResult()
                //    {
                //        Result = (int) KTSkillManager.UseSkillResult.None,
                ...
                if (skillID != -1)
                {
                    ...
                    else if (status == 1)
                    {
                        ...
                        KTSkillManager.UseSkill(client, null, skill);
                    }
                }
                else
                {
        ...
        public static void SendSaveQuickKeyToDB(KPlayer player)
        {
            ...
            GameManager.DBCmdMgr.AddDBCmd((int) TCPGameServerCmds.CMD_DB_UPDATEKEYS, player.RoleID + ":" + 0 + ":" + cmdData, null, player.ServerId);
        }
        ...
        public static void SendSaveAruaKeyToDB(KPlayer player)
        {
            ...
            GameManager.DBCmdMgr.AddDBCmd((int) TCPGameServerCmds.CMD_DB_UPDATEKEYS, player.RoleID + ":" + 1 + ":" + cmdData, null, player.ServerId);
        }
        ...
        public static TCPProcessCmdResults ResponseUseSkill(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                /// Nếu đang bày bán thì không cho dùng kỹ năng
                else if (client.StallDataItem != null)
                {
                    PlayerManager.ShowNotification(client, "Đang trong trạng thái bán hàng, không thể sử dụng kỹ năng!");
                    /// Gói lại gửi Client
                    byte[] _resultData = DataHelper.ObjectToBytes<G2C_UseSkillResult>(new G2C_UseSkillResult()
				...
				{
                    PlayerManager.ShowNotification(client, "Bạn hiện không thể sử dụng kỹ năng!");
                    /// Gói lại gửi Client
                    byte[] _resultData = DataHelper.ObjectToBytes<G2C_UseSkillResult>(new G2C_UseSkillResult()
                    {
                        Result = (int) KTSkillManager.UseSkillResult.None,
                ...
                {
                    PlayerManager.ShowNotification(client, "Trong khu an toàn không thể sử dụng kỹ năng tấn công mục tiêu!");
                    /// Gói lại gửi Client
                    byte[] _resultData = DataHelper.ObjectToBytes<G2C_UseSkillResult>(new G2C_UseSkillResult()
                    {
                        Result = (int) KTSkillManager.UseSkillResult.None,
                ...
                if (useSkill.SkillID == 10)
				{
                    ...
                    if (!client.IsCanMove())
					{
                        PlayerManager.ShowNotification(client, "Đang trong trạng thái bị không thể sử dụng kỹ năng!");
                        /// Gói lại gửi Client
                        byte[] _resultData = DataHelper.ObjectToBytes<G2C_UseSkillResult>(new G2C_UseSkillResult()
                        {
                            Result = (int) KTSkillManager.UseSkillResult.None,
                ...
                Interface.IObject obj = Global.FindSpriteByID(useSkill.TargetID, client.CurrentMapCode);
                ...
                /// Dùng kỹ năng
                KTSkillManager.UseSkillResult result = KTSkillManager.UseSkill(client, target, skillRef);

                /// Gói lại gửi Client
                byte[] resultData = DataHelper.ObjectToBytes<G2C_UseSkillResult>(new G2C_UseSkillResult()
                {
                    Result = (int) result,
                    PosX = posX,
                    PosY = posY,
        ...
        public static void NotifySkillCooldown(KPlayer player, SkillCooldown skill)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClient(player, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_NOTIFYSKILLCOOLDOWN);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void ResetSkillCooldown(KPlayer player, List<int> excludedSkills = null)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClient(player, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_RESETSKILLCOOLDOWN);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendObjectUseSkill(GameObject go, SkillDataEx skill, bool isSpecialAttack)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_USESKILL);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendCreateBullet(GameObject caster, int bulletID, int resID, UnityEngine.Vector2 fromPos, UnityEngine.Vector2 toPos, GameObject chaseTarget, bool isTrap, int velocity, float lifeTime, bool loopAnimation, float delay)
        {
            try
            {
                ...
                /// Tìm tất cả người chơi xung quanh để gửi gói tin
                List<object> listObjects = Global.GetAll9Clients(caster);
                if (listObjects == null)
                {
                    return;
                }
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_CREATEBULLET);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendCreateBullets(GameObject go, List<G2C_CreateBullet> packets)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_CREATEBULLETS);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendBulletExplode(GameObject go, int bulletID, int resID, UnityEngine.Vector2 pos, float delay, GameObject target)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_BULLETEXPLODE);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendBulletExplodes(GameObject go, List<G2C_BulletExplode> explodes)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_BULLETEXPLODES);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        /// <param name="stateEffects">Danh sách Buff</param>
        /// <param name="elementEffects">Danh sách trạng thái ngũ hành</param>
        public static void SendSkillResult(GameObject caster, GameObject target, KTSkillManager.SkillResult type, int damage)
        {
            try
            {
                ...
                /// Tìm tất cả người chơi xung quanh để gửi gói tin
                List<object> listObjects = Global.GetAll9Clients(target);
                if (listObjects == null)
                {
                    return;
                }
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_SKILLRESULT);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendSkillResults(GameObject target, List<SkillResult> skillResults)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_SKILLRESULTS);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendBlinkToPosition(GameObject caster, int posX, int posY, float duration)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_BLINKTOPOSITION);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendFlyToPosition(GameObject caster, int posX, int posY, float duration)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_FLYTOPOSITION);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendSpriteAddBuff(GameObject target, BuffDataEx buff)
        {
            try
            {
                ...
                if (target.IsPlayer())
                {
                    GameManager.ClientMgr.SendToClient(target as KPlayer, cmdDataMyself, (int) TCPGameServerCmds.CMD_KT_G2C_SPRITEBUFF);
                }
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, target, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_SPRITEBUFF);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendSpriteRemoveBuff(GameObject target, BuffDataEx buff)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_SPRITEBUFF);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void NotifyTargetMoveSpeedChanged(GameObject target)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_MOVESPEEDCHANGED);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void NotifyTargetAttackSpeedChanged(GameObject target)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_ATTACKSPEEDCHANGED);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void SendObjectInvisibleState(GameObject target, int type)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_OBJECTINVISIBLESTATECHANGED);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
