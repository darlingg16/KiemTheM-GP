    ...
    public static partial class KT_TCPHandler
    {
        ...
        public static void NotifySpriteChangeAction(GameObject target)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_CHANGEACTION);
            }
            catch (Exception ex)
            {
        ...
        public static void NotifySpriteSeriesState(GameObject target, KE_STATE state, int type, float timeSecond = 0, int dragPosX = 0, int dragPosY = 0)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_SPRITESERIESSTATE);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static TCPProcessCmdResults ResponseToggleHorseState(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                if (horseGD == null)
                {
                    PlayerManager.ShowNotification(client, "Không có ngựa cưỡi!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                if (KTGlobal.GetCurrentTimeMilis() - client.LastTickToggleHorseState < KTGlobal.TickHorseStateChange)
                {
                    ...
                    PlayerManager.ShowNotification(client, string.Format("Không thể thay đổi trạng thái cưỡi ngựa lúc này, hãy chờ {0} giây rồi thử lại!", sec));
                    return TCPProcessCmdResults.RESULT_OK;
                }
        ...
        public static void SendHorseStateChanged(KPlayer client)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_TOGGLE_HORSE_STATE);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
