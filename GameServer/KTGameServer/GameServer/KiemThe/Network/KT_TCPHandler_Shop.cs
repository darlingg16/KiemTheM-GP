    ...
    public static partial class KT_TCPHandler
    {
        ...
        public static void SendShopData(KPlayer client, ShopTab _ShopData)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClient(client, cmdData, (int)TCPGameServerCmds.CMD_KT_C2G_OPENSHOP);
            }
            catch (Exception ex)
            {
        ...
        public static TCPProcessCmdResults ResponseOpenTokenShop(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                GameManager.ClientMgr.SendToClient(client, bytesData, (int)TCPGameServerCmds.CMD_KT_OPEN_TOKENSHOP);
        ...
        public static TCPProcessCmdResults ProcessSpriteNPCBuyCmd(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    PlayerManager.ShowNotification(client, "Số lượng không hợp lệ");
        ...
        public static TCPProcessCmdResults ProcessSpriteNPCSaleOutCmd(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                if (gameMap.MapType != "village" && gameMap.MapType != "faction" && gameMap.MapType != "city" && gameMap.MapType != "xoyogame_waitingroom" && gameMap.MapType != "factionbattle" && gameMap.MapType != "xisuidao" && gameMap.MapType != "xiaoyaogubaomingdian" && gameMap.MapType != "liansaihuichang" && gameMap.MapType != "qinshihuangling_1" && gameMap.MapType != "qinshihuangling_3")
                {
                    /// Cho toác luôn
                    return TCPProcessCmdResults.RESULT_FAILED;
                }
                ...
                ///// Nếu vị trí quá xa
                //if (Global.GetTwoPointDistance(client.CurrentPos, client.LastShopNPC.CurrentPos) > 100)
				//{
                //    PlayerManager.ShowNotification(client, "Khoảng cách tới NPC quá xa!");
                //    return TCPProcessCmdResults.RESULT_OK;
				//}
                ...
                GoodsData ItemSell = Global.GetGoodsByDbID(client, ItemIDDb);
                ...
                if (ItemSell != null)
                {
                    ...
                    if (_Item != null)
                    {
						...
						{
                            PlayerManager.ShowNotification(client, "Vật phẩm này không bán được!");
                            /// Không cho bán
                            return TCPProcessCmdResults.RESULT_OK;
                        }
						...
						{
                            PlayerManager.ShowNotification(client, "Trang bị đã cường hóa trên cấp 8, không thể bán được!");
                            /// Không cho bán
                            return TCPProcessCmdResults.RESULT_OK;
                        }

                        else if (ItemSell.Endtime != Global.ConstGoodsEndTime)
                        {
                            PlayerManager.ShowNotification(client, "Đồ có hạn sử dụng không thể bán!");
                            /// Không cho bán
                            return TCPProcessCmdResults.RESULT_OK;
                        }
