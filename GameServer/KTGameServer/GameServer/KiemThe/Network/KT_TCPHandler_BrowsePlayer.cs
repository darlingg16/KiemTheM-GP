    ...
    public static partial class KT_TCPHandler
    {
        ...
        public static TCPProcessCmdResults ResponseBrowsePlayers(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    PlayerManager.ShowNotification(client, "Tên người chơi chứa ký tự đặc biệt!");
                ...
                {
                    PlayerManager.ShowNotification(client, "Chỉ được phép tìm kiếm người chơi mỗi 5 giây!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                List<KPlayer> players = GameManager.ClientMgr.FindClients(x => x.RoleName.ToLower().Contains(playerName));
        ...
        public static TCPProcessCmdResults ResponseCheckPlayerLocation(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                GoodsData itemGD = client.GoodsDataList?.Where(x => KTGlobal.CheckPositionScroll.Contains(x.GoodsID) && x.GCount > 0).FirstOrDefault();
                if (itemGD == null)
				{
                    PlayerManager.ShowNotification(client, "Cần vật phẩm [Quan Thiên Quyển] mới có thể sử dụng chức năng này!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
