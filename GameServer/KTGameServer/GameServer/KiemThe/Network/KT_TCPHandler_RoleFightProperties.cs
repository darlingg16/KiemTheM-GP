    ...
    public static partial class KT_TCPHandler
    {
        ...
        public static void SendObjectDiedToClients(GameObject go)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_SPR_DEAD);
            }
            catch (Exception ex)
            {
        ...
        public static void NotifyAttribute(KPlayer client)
        {
            try
            {
                RoleAttributes _Atrtribute = client.GetRoleAttributes();
                KeyValuePair<RoleAttributes, bool> pairData = new KeyValuePair<RoleAttributes, bool>(_Atrtribute, false);
                byte[] cmdData = DataHelper.ObjectToBytes(pairData);
                GameManager.ClientMgr.SendToClient(client, cmdData, (int) TCPGameServerCmds.CMD_KT_ROLE_ATRIBUTES);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void NotifySelfLifeChanged(KPlayer player, int hp)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClient(player, cmdData, (int) TCPGameServerCmds.CMD_SPR_UPDATE_ROLEDATA);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void NotifyObjectLifeChangedToAllPlayers(GameObject go, int hp)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, go, listObjects, cmdData, (int) TCPGameServerCmds.CMD_SPR_UPDATE_ROLEDATA);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void NotifySelfGatherMakePointChanged(KPlayer player)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClient(player, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_UPDATE_ROLE_GATHERMAKEPOINT);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void NotifySelfLifeSkillLevelAndExpChanged(KPlayer player, int id, int level, int exp)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClient(player, cmdData, (int) TCPGameServerCmds.CMD_KT_G2C_UPDATE_LIFESKILL_LEVEL);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static void NotifySelfRoleValueChanged(KPlayer player, long value)
        {
            try
            {
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, cmdData, (int) TCPGameServerCmds.CMD_KT_UPDATE_TOTALVALUE);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
        ...
        public static TCPProcessCmdResults ResponseGetOtherRoleEquipInfo(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                /// Thông tin
                RoleData roleData = KTLogic.GetOtherPlayerRoleData(otherClient);
        ...
        #region Bang hội và gia tộc chức vụ
        /// <summary>
        /// Gửi gói tin thông báo chức vụ bang hội, gia tộc và quan hàm của đối tượng thay đổi tới tất cả người chơi xung quanh
        /// </summary>
        /// <param name="player"></param>
        public static void NotifyOtherMyGuildAndFamilyRankChanged(KPlayer player)
        {
            try
            {
                string strCmd = string.Format("{0}:{1}:{2}:{3}:{4}:{5}", player.RoleID, player.GuildID, player.GuildRank, player.FamilyID, player.FamilyRank, player.OfficeRank);
                ...
                /// Gửi gói tin đến tất cả người chơi xung quanh
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, strCmd, (int) TCPGameServerCmds.CMD_KT_UPDATE_GUILDANDFAMILY_RANK);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
