    ...
    public static partial class KT_TCPHandler
    {
        ...
        public static TCPProcessCmdResults ResponseCreateTeam(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    PlayerManager.ShowNotification(client, "Bản đồ này không cho phép tạo nhóm!");
                ...
                {
                    PlayerManager.ShowNotification(client, "Bạn đã có nhóm, không thể tạo thêm!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                GameManager.ClientMgr.SendToClient(client, rspString, nID);
        ...
        public static TCPProcessCmdResults ResponseInviteTeammate(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    PlayerManager.ShowNotification(client, "Bản đồ này không cho phép mời vào nhóm!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                if (client.TeamID == -1 || !KTTeamManager.IsTeamExist(client.TeamID))
                {
                    PlayerManager.ShowNotification(client, "Bạn chưa có nhóm, không thể thêm thành viên!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                /// Nếu không phải đội trưởng
                else if (client.TeamLeader != client)
                {
                    PlayerManager.ShowNotification(client, "Bạn không phải đội trưởng, không thể thao tác!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                else if (client.Teammates.Count >= KTTeamManager.MaxTeamSize)
                {
                    PlayerManager.ShowNotification(client, "Nhóm đã đầy, không thể thêm thành viên!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                {
                    PlayerManager.ShowNotification(client, "Không thể tự gửi lời mời tới chính mình!");
                    return TCPProcessCmdResults.RESULT_OK;
                }

                /// Người chơi tương ứng được thêm vào nhóm
                KPlayer player = GameManager.ClientMgr.FindClient(playerID);
                ...
                {
                    PlayerManager.ShowNotification(client, "Người chơi đã ở trong nhóm rồi!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                /// Nếu người chơi đã trong nhóm khác
                else if (player.TeamID != -1 && KTTeamManager.IsTeamExist(player.TeamID))
                {
                    PlayerManager.ShowNotification(client, "Người chơi đã ra nhập nhóm khác!");
                    return TCPProcessCmdResults.RESULT_OK;
                }

                /// Gửi yêu cầu mời vào nhóm đến người chơi tương ứng
                string strCmd = string.Format("{0}:{1}:{2}", client.TeamID, client.RoleID, client.RoleName);
                GameManager.ClientMgr.SendToClient(player, strCmd, (int)TCPGameServerCmds.CMD_KT_INVITETOTEAM);

                PlayerManager.ShowNotification(client, "Gửi lời mời vào nhóm tới " + player.RoleName + " thành công!");
                return TCPProcessCmdResults.RESULT_OK;
            }
            catch (Exception ex)
            {
        ...
        public static TCPProcessCmdResults ResponseAgreeJoinTeam(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    PlayerManager.ShowNotification(client, "Bản đồ này không cho phép gia nhập nhóm!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                {
                    PlayerManager.ShowNotification(client, "Nhóm không tồn tại hoặc đã giải tán!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                {
                    /// Người chơi tương ứng
                    player = GameManager.ClientMgr.FindClient(playerID);
                    /// Nếu người chơi không tồn tại
                    if (player == null)
                    {
                        PlayerManager.ShowNotification(client, "Người chơi không tồn tại hoặc đã rời!");
                        return TCPProcessCmdResults.RESULT_OK;
                    }
                    ...
                    {
                        PlayerManager.ShowNotification(client, "Người chơi này đã là thành viên tổ đội rồi!");
                        return TCPProcessCmdResults.RESULT_OK;
                    }
                    /// Nếu người chơi đã có nhóm
                    else if (player.TeamID != -1 && KTTeamManager.IsTeamExist(player.TeamID))
                    {
                        PlayerManager.ShowNotification(client, "Người chơi này đã gia nhập nhóm khác!");
                        return TCPProcessCmdResults.RESULT_OK;
                    }
                }
                else
                {
                    ...
                    {
                        PlayerManager.ShowNotification(client, "Bạn đã ở trong nhóm này rồi!");
                        return TCPProcessCmdResults.RESULT_OK;
                    }
                    /// Nếu đã có nhóm
                    else if (client.TeamID != -1 && KTTeamManager.IsTeamExist(client.TeamID))
                    {
                        PlayerManager.ShowNotification(client, "Bạn đã có nhóm, không thể gia nhập nhóm khác cho đến khi rời nhóm hiện tại!");
                        return TCPProcessCmdResults.RESULT_OK;
                    }
                }
                ...
                {
                    RoleDataMini roleData = Global.ClientToRoleDataMini(teammate, false);
                ...
                GameManager.ClientMgr.SendToClient(player, __cmdData, (int)TCPGameServerCmds.CMD_KT_AGREEJOINTEAM);
        ...
        public static TCPProcessCmdResults ResponseLeaveTeam(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    PlayerManager.ShowNotification(client, "Bản đồ này không cho phép chủ động rời khỏi nhóm!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
        ...
        public static TCPProcessCmdResults ResponseKickOutTeammate(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    PlayerManager.ShowNotification(client, "Bản đồ này không cho phép trục xuất thành viên khỏi nhóm!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                {
                    PlayerManager.ShowNotification(client, "Bạn không phải đội trưởng, không thể thực hiện thao tác này!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                /// Nếu ID người chơi bị đuổi là bản thân
                else if (client.RoleID == kickedOutTeammateID)
                {
                    PlayerManager.ShowNotification(client, "Không thể tự trục xuất bản thân mình ra khỏi nhóm!");
                    return TCPProcessCmdResults.RESULT_OK;
                }

                /// Người chơi bị đuổi khỏi nhóm
                KPlayer kickOutPlayer = GameManager.ClientMgr.FindClient(kickedOutTeammateID);
                ...
                foreach (KPlayer teammate in client.Teammates)
                {
                    ...
                    byte[] _cmdData = DataHelper.ObjectToBytes<TeamMemberChanged>(teamMember);
                    GameManager.ClientMgr.SendToClient(teammate, _cmdData, (int)TCPGameServerCmds.CMD_KT_KICKOUTTEAMMATE);
                }
                ...
                GameManager.ClientMgr.SendToClient(kickOutPlayer, __cmdData, (int)TCPGameServerCmds.CMD_KT_KICKOUTTEAMMATE);
        ...
        public static TCPProcessCmdResults ResponseRefuseJoinTeam(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    fromPlayer = GameManager.ClientMgr.FindClient(fromPlayerID);
                ...
                {
                    toPlayer = GameManager.ClientMgr.FindClient(fromPlayerID);
                ...
                GameManager.ClientMgr.SendToClient(fromPlayer, _cmdData, (int)TCPGameServerCmds.CMD_KT_REFUSEJOINTEAM);
                GameManager.ClientMgr.SendToClient(toPlayer, _cmdData, (int)TCPGameServerCmds.CMD_KT_REFUSEJOINTEAM);
        ...
        public static TCPProcessCmdResults ResponseApproveTeamLeader(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    PlayerManager.ShowNotification(client, "Bản đồ này không cho phép thay đổi nhóm trưởng!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                if (client.TeamID == -1 || !KTTeamManager.IsTeamExist(client.TeamID))
                {
                    PlayerManager.ShowNotification(client, "Bạn không có tổ đội!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                /// Nếu bản thân không phải đội trưởng
                else if (client != client.TeamLeader)
                {
                    PlayerManager.ShowNotification(client, "Bạn không phải đội trưởng, không thể bổ nhiệm thành viên khác làm đội trưởng!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                if (null == player)
                {
                    PlayerManager.ShowNotification(client, "Đội viên không tồn tại hoặc đã rời mạng!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                if (player == client)
                {
                    PlayerManager.ShowNotification(client, "Bạn không thể bổ nhiệm chính mình làm đội trưởng!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
        ...
        public static TCPProcessCmdResults ResponseAskToJoinTeam(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                if (client.TeamID != -1 && KTTeamManager.IsTeamExist(client.TeamID))
                {
                    PlayerManager.ShowNotification(client, "Bạn đã có tổ đội, không thể xin gia nhập tổ đội khác!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                if (player == client)
                {
                    PlayerManager.ShowNotification(client, "Có lỗi xảy ra. Hãy báo cáo với hỗ trợ viên để được giải đáp!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                {
                    PlayerManager.ShowNotification(client, "Đối phượng không có tổ đội, không thể gia nhập!");
                    return TCPProcessCmdResults.RESULT_OK;
                }
                ...
                GameManager.ClientMgr.SendToClient(teamLeader, _cmdData, (int)TCPGameServerCmds.CMD_KT_ASKTOJOINTEAM);
        ...
        public static TCPProcessCmdResults ResponseGetTeamInfo(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                GameManager.ClientMgr.SendToClient(client, _cmdData, nID);
        ...
        public static void SendRefreshTeamMemberAttrib(KPlayer player)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClient(player, cmdData, (int)TCPGameServerCmds.CMD_KT_REFRESHTEAMMEMBERATTRIBUTES);
            }
            catch (Exception ex)
            {
        ...
        public static void SendRefreshTeamMemberAttribToAllTeammates(KPlayer player)
        {
            try
            {
                ...
                foreach (KPlayer teammate in player.Teammates)
                {
                    GameManager.ClientMgr.SendToClient(teammate, cmdData, (int)TCPGameServerCmds.CMD_KT_REFRESHTEAMMEMBERATTRIBUTES);
                }
            }
            catch (Exception ex)
            {
        ...
        public static void SendSpriteTeamChangedToAllOthers(KPlayer player)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClients(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, null, listObjects, strCmd, (int)TCPGameServerCmds.CMD_KT_G2C_UPDATESPRITETEAMDATA);
            }
            catch (Exception ex)
            {
                LogManager.WriteLog(LogTypes.Exception, ex.ToString());
