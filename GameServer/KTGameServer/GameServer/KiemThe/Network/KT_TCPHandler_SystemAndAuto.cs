    ...
    public static partial class KT_TCPHandler
    {
        ...
        public static TCPProcessCmdResults ResponseClientAutoPathTransferMap(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                KPlayer player = GameManager.ClientMgr.FindClient(socket);
                ...
                if (useNPC)
                {
                    ...
                    if (KTAutoPathManager.CanTransferFromNPCToMap(player, toMapCode, out AutoPathXML.Node node))
                    {
                        /// Nếu ID bản đồ đích khác bản đồ hiện tại
                        if (player.CurrentMapCode != toMapCode)
                        {
                            /// Syns tọa độ
                            GameManager.ClientMgr.NotifyChangeMap(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, player, node.ToMapCode, node.ToX, node.ToY, (int) player.CurrentDir);
                        }
                        /// Nếu ID bản đồ đích trùng với bản đồ hiện tại
                        else
                        {
                            GameManager.ClientMgr.NotifyOthersGoBack(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, player, node.ToX, node.ToY, (int) player.CurrentDir);
                        }
                    }
                    /// Nếu không thấy NPC dịch chuyển thì toang
                    else
                    {
                        PlayerManager.ShowNotification(player, "Không tìm thấy NPC dịch chuyển xung quanh!");
                    }
                }
                /// Nếu là dịch từ truyền tống phù
                else
                {
                    GameMap gameMap = GameManager.MapMgr.GetGameMap(player.CurrentMapCode);
                    /// Nếu bản đồ không cho phép dùng vật phẩm
                    if (gameMap != null && !gameMap.AllowUseItem)
                    {
                        PlayerManager.ShowNotification(player, "Bản đồ này không cho phép sử dụng vật phẩm!");
                        return TCPProcessCmdResults.RESULT_OK;
                    }
                    else if (gameMap != null && gameMap.BanItems.Contains(itemID))
					{
                        ...
                        {
                            PlayerManager.ShowNotification(player, "Tại liên máy chủ AUTO không thể sử dụng Truyền Tống Phù.Vui lòng sử dụng xa phu để di chuyển");
                            return TCPProcessCmdResults.RESULT_OK;
                        }
                        else
                        {
                            PlayerManager.ShowNotification(player, "Bản đồ này không cho phép sử dụng Truyền Tống Phù!");
                            return TCPProcessCmdResults.RESULT_OK;
                        }
                    ...
                    GoodsData itemGD = player.GoodsDataList.Where(x => x.GoodsID == itemID).OrderByDescending(x => x.Binding).FirstOrDefault();
                    /// Nếu vật phẩm không tồn tại
                    if (itemGD == null)
                    {
                        PlayerManager.ShowNotification(player, "Không có truyền tống phù!");
                        return TCPProcessCmdResults.RESULT_OK;
                    }

                    /// Nếu tìm thấy điểm dịch chuyển bằng truyền tống phù vị trí tương ứng đang đứng
                    if (KTAutoPathManager.CanTransferByUsingTeleportItem(player, itemID, toMapCode, out AutoPathXML.Node node))
                    {
                        /// Nếu ID bản đồ đích trùng với bản đồ hiện tại
                        if (player.CurrentMapCode != toMapCode)
                        {
                    ...
                    {
                        PlayerManager.ShowNotification(player, "Không tìm thấy vị trí dịch chuyển xung quanh!");
                    }
                }
