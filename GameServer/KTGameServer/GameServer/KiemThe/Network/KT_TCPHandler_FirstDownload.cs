    ...
    public static partial class KT_TCPHandler
    {
        ...
        public static void SendDownloadBonus(KPlayer client)
        {
            try
            {
                ...
                GameManager.ClientMgr.SendToClient(client, bytesCmd, (int) TCPGameServerCmds.CMD_KT_GET_BONUS_DOWNLOAD);
            }
            catch (Exception ex)
            {
        ...
        public static TCPProcessCmdResults ProcessDownloadBonus(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                if (client.ReviceBounsDownload == 0)
                {
                    ...
                    {
                        PlayerManager.ShowNotification(client, "Nhận thưởng thành công!");
                        client.ReviceBounsDownload = 1;
                        foreach (BonusItem _item in ConfigSend.BonusItems)
                        {
                            if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, client, _item.ItemID, _item.Number, 0, "BOUNDSREWARD", false, 1, false, Global.ConstGoodsEndTime,"",-1,"",0,1,false))
                            {
                                PlayerManager.ShowNotification(client, "Có lỗi khi thêm vật phẩm!");
                            }
                        }
                    }
                    else
                    {
                        PlayerManager.ShowNotification(client, "Túi không đủ chỗ!");
                    }
                }
                else
                {
                    PlayerManager.ShowNotification(client, "Bạn đã nhận phần thưởng này rồi!");
                }
