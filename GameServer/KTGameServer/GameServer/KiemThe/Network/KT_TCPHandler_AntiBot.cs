        ...
        public static TCPProcessCmdResults ResponseCaptchaAnswer(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                if (client.CurrentCaptcha.Text != cmdData)
                {
                    ...
                    {
                        PlayerManager.ShowNotification(client, "Câu trả lời không chính xác, thẩm vấn lại sau!");
                    }
                    /// Nếu không phải đang ở nhà lao
                    else
                    {
                        PlayerManager.ShowNotification(client, "Câu trả lời không chính xác, tự chuyển vào nhà lao!");
                    }
                ...
                if (client.CurrentMapCode == KTGlobal.JailMapCode)
                {
                    PlayerManager.ShowNotification(client, "Câu trả lời chính xác. Ngươi có thể rời đi rồi!");
                }
                /// Nếu không phải đang ở nhà lao
                else
                {
                    PlayerManager.ShowNotification(client, "Câu trả lời chính xác. Cáo lỗi đã làm phiền bằng hữu!");
                }
                ...
                /// Thêm bạc khóa
                GameManager.ClientMgr.AddUserBoundMoney(client, ServerConfig.Instance.CaptchaBoundMoneyAddPerLevel * nLevel, "AnswerCaptcha");
                /// Thêm kinh nghiệm
                GameManager.ClientMgr.ProcessRoleExperience(client, ServerConfig.Instance.CaptchaExpAddPerLevel * nLevel, true, false, true, "AnswerCaptcha");
                /// Xóa Captcha
                client.RemoveCaptcha();
                /// Thực thi hàm khi trả lời sai
