...
{
	/// <summary>
	/// Script ải Tiêu Dao Cốc - tránh bẫy, giết quái và Boss
	/// </summary>
	public class XoYo_Script_HideTrapAndKillMonster : CopySceneEvent
	{
		#region Constants
		/// <summary>
		...
		/// <summary>
		/// Danh sách bẫy đang chờ
		/// </summary>
		private readonly List<XoYo.TrapInfo> waitToBeSpawnedTraps = new List<XoYo.TrapInfo>();
		...
		public XoYo_Script_HideTrapAndKillMonster(KTCopyScene copyScene, XoYo.MapInfo mapInfo) : base(copyScene)
		{
			...
			/// Danh sách bẫy được sinh ra theo thời gian
			this.waitToBeSpawnedTraps = this.MapInfo.Traps.Where(x => x.SpawnAfter > 0).ToList();
		}
		...
		protected override void OnTick()
		{
			...
			if (this.LifeTimeTicks <= XoYo.BeginWaitTime)
			{
				...
				if (KTGlobal.GetCurrentTimeMilis() - this.LastNotifyTick >= XoYo_Script_HideTrapAndKillMonster.NotifyActivityInfoToPlayersEveryTick)
				{
			...
			else
			{
				...
				if (this.nStep == 0)
				{
					...
					/// Sinh bẫy
					this.CreateImmediateTraps();
					/// Nếu không có quái
					if (this.MapInfo.Monsters.Count <= 0)
					{
						/// Sinh Boss
				...
				else if (this.nStep == 1)
				{
					...
					/// Tự xóa bẫy hết thời gian
					this.RemoveTimeoutTraps();
				}
				/// Nếu Step = 2
		...
		/// <summary>
		/// Tạp bẫy sinh ra từ đầu
		/// </summary>
		private void CreateImmediateTraps()
		{
			/// Duyệt danh sách quái
			foreach (XoYo.TrapInfo trapInfo in this.MapInfo.Traps)
			{
				/// Nếu sinh ra ngay lập tức
				if (trapInfo.SpawnAfter == -1)
				{
					/// Ngũ hành
					KE_SERIES_TYPE series = KE_SERIES_TYPE.series_none;
					/// Hướng quay
					...
					int hp = 1;
					/// Tạo quái
					GameManager.MonsterZoneMgr.AddDynamicMonsters(this.CopyScene.MapCode, trapInfo.ID, this.CopyScene.ID, 1, trapInfo.PosX, trapInfo.PosY, "", "", hp, this.Level, dir, series, MonsterAIType.Normal, -1, -1, -1, "Trap", (monster) => {
						/// Vô địch
						monster.m_CurrentInvincibility = 1;
						/// Đánh dấu là quái tĩnh
						monster.IsStatic = true;
						
						/// Nếu có vòng sáng
						if (trapInfo.Auras.Count > 0)
						{
							/// Duyệt danh sách vòng sáng
							foreach (SkillLevelRef aura in trapInfo.Auras)
							{
								/// Kích hoạt vòng sáng
								monster.UseSkill(aura.SkillID, aura.Level, monster);
							}
						...
						/// Thiết lập thời điểm tạo ra
						monster.SetLocalParam(10000, KTGlobal.GetCurrentTimeMilis());
						/// Thiết lập thời gian tự hủy
						monster.SetLocalParam(10001, trapInfo.DestroyAfter);
					}, 65535);
				}
			}
		}
		...
		private void CreateObjectsInTime()
		{
			...
			/// Nếu tồn tại danh sách chờ thêm
			if (this.waitToBeSpawnedTraps.Count > 0)
			{
				/// Danh sách quái cần xóa
				List<XoYo.TrapInfo> toRemoveWaitingTraps = null;
				/// Duyệt danh sách quái
				foreach (XoYo.TrapInfo trapInfo in this.waitToBeSpawnedTraps)
				{
					/// Nếu đã đến thời gian sinh
					if (this.LifeTimeTicks >= trapInfo.SpawnAfter + XoYo.BeginWaitTime)
					{
						/// Thêm vào danh sách cần xóa
						if (toRemoveWaitingTraps == null)
						{
							toRemoveWaitingTraps = new List<XoYo.TrapInfo>();
						}
						toRemoveWaitingTraps.Add(trapInfo);
				...
				{
					/// Duyệt danh sách cần xóa
					foreach (XoYo.TrapInfo trapInfo in toRemoveWaitingTraps)
					{
						/// Xóa khỏi danh sách
						this.waitToBeSpawnedTraps.Remove(trapInfo);
					}
		...
		/// Xóa toàn bộ bẫy
		/// </summary>
		private void RemoveAllTraps()
		{
			/// Duyệt danh sách quái
			foreach (Monster monster in this.GetMonsters())
			{
				/// Nếu là bẫy và đã
				if (monster.Tag != null && monster.Tag == "Trap")
				{
		...
		/// <summary>
		/// Xóa toàn bộ các bẫy đã hết thời gian
		/// </summary>
		private void RemoveTimeoutTraps()
		{
			/// Duyệt danh sách quái
			foreach (Monster monster in this.GetMonsters())
			{
				/// Nếu là bẫy và đã hết thời gian
				if (monster.Tag != null && monster.Tag == "Trap" && monster.GetLocalParam(10001) != -1 && KTGlobal.GetCurrentTimeMilis() - monster.GetLocalParam(10000) >= monster.GetLocalParam(10001))
				{
					this.RemoveMonster(monster);
				}
			}
