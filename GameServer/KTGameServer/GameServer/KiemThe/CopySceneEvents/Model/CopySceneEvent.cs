...
namespace GameServer.KiemThe.CopySceneEvents.Model
{
	...
	public abstract class CopySceneEvent
	{
        ...
        /// </summary>
        private readonly List<DelayTask> delayTasks = new List<DelayTask>();
        ...
        public virtual void OnPlayerLeave(KPlayer player, GameMap toMap)
		{
            ...
            KT_TCPHandler.RecordCopySceneInfoToDB(player, -1, -1, -1, -1, -1);
        }
        ...
        private void ExecuteTick(object o, DoWorkEventArgs e)
        {
            ...
            List<DelayTask> toRemoveTasks = null;
            /// Duyệt danh sách DelayTask
            foreach (DelayTask delayTask in this.delayTasks)
			{
                ...
                if (delayTask.InTime)
				{
					...
					{
                        toRemoveTasks = new List<DelayTask>();
                    }
                    toRemoveTasks.Add(delayTask);
            ...
            if (toRemoveTasks != null)
			{
                ...
                foreach (DelayTask delayTask in toRemoveTasks)
				{
                    /// Xóa khỏi danh sách gốc
                    this.delayTasks.Remove(delayTask);
				}
			}
        ...
        private void MovePlayersToCopyScene(List<KPlayer> players)
        {
            ...
            foreach (KPlayer player in players)
            {
                ...
                GameManager.ClientMgr.NotifyChangeMap(TCPManager.getInstance().MySocketListener, TCPOutPacketPool.getInstance(), player, this.CopyScene.MapCode, this.CopyScene.EnterPosX, this.CopyScene.EnterPosY, 0, true);

                /// Lưu thông tin vị trí trước đó của người chơi
                KT_TCPHandler.RecordCopySceneInfoToDB(player, this.CopyScene.ID, player.CurrentMapCode, (int) player.CurrentPos.X, (int) player.CurrentPos.Y, this.CopyScene.InitTicks);
        ...
        private void MovePlayersOutOfCopyScene()
        {
            ...
            foreach (KPlayer player in this.GetPlayers())
            {
                ...
                GameManager.ClientMgr.NotifyChangeMap(TCPManager.getInstance().MySocketListener, TCPOutPacketPool.getInstance(), player, this.CopyScene.OutMapCode, this.CopyScene.OutPosX, this.CopyScene.OutPosY, 0, true);
                /// Đánh dấu ID phụ bản
                player.CopyMapID = -1;
                /// Lưu thông tin vị trí trước đó của người chơi
                KT_TCPHandler.RecordCopySceneInfoToDB(player, -1, -1, -1, -1, -1);
        ...
        private void ClearCopySceneObjects()
        {
            ...
            NPCGeneralManager.RemoveMapNpcs(this.CopyScene.MapCode, this.CopyScene.ID);
        ...
        {
            return GameManager.ClientMgr.GetMapClientsCount(this.CopyScene.MapCode, this.CopyScene.ID);
        }
        ...
        {
            /// Trả về kết quả
            return GameManager.ClientMgr.GetMapClients(this.CopyScene.MapCode, this.CopyScene.ID);
        }
        ...
        {
            /// Trả về kết quả
            return GameManager.MonsterMgr.GetMonstersCount(this.CopyScene.MapCode, this.CopyScene.ID);
        }

        /// <summary>
        /// Trả về tổng số quái trong bản đồ thỏa mãn điều kiện tương ứng
        /// </summary>
        /// <param name="predicate"></param>
        /// <returns></returns>
        protected int GetTotalMonsters(Predicate<Monster> predicate)
		{
            /// Trả về kết quả
            return GameManager.MonsterMgr.GetMonstersCount(predicate, this.CopyScene.MapCode, this.CopyScene.ID);
        }
        ...
        {
            /// Trả về kết quả
            return GameManager.MonsterMgr.GetObjectsByMap(this.CopyScene.MapCode, this.CopyScene.ID);
        }
        ...
        {
            return NPCGeneralManager.GetMapNPCs(this.CopyScene.MapCode, this.CopyScene.ID).Count;
        }
        ...
        protected List<NPC> GetNPCs()
        {
            ...
            return NPCGeneralManager.GetMapNPCs(this.CopyScene.MapCode, this.CopyScene.ID);
        }
        ...
        {
            NPCGeneralManager.RemoveMapNpc(npc.MapCode, npc.CopyMapID, npc.NPCID);
        }
		...
		{
            this.delayTasks.Add(new DelayTask()
            {
                Delay = delay,
        ...
        protected void NotifyAllPlayers(string message)
        {
            foreach (KPlayer player in this.GetPlayers())
            {
                PlayerManager.ShowNotification(player, message);
            }
        }
