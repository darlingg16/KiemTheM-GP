...
namespace GameServer.KiemThe.CopySceneEvents.Family
{
	...
	public static class FamilyFuBen_EventScript
	{
		...
		{
			string cmdData = string.Format("{0}:{1}:{2}", 0, familyID, -1);
			string[] dbFields = Global.ExecuteDBCmd((int) TCPGameServerCmds.CMD_KT_FAMILY_WEEKLYFUBENCOUNT, cmdData, GameManager.LocalServerId);
			if (null == dbFields || dbFields.Length != 1)
			{
				LogManager.WriteLog(LogTypes.Error, "GetTotalOpenedFuBenThisWeek of family ID = " + familyID + " faild!");
				return 0;
			}
			else
			{
				return int.Parse(dbFields[0]);
			}
		}
		...
		{
			string cmdData = string.Format("{0}:{1}:{2}", 1, familyID, nTimes);
			Global.ExecuteDBCmd((int) TCPGameServerCmds.CMD_KT_FAMILY_WEEKLYFUBENCOUNT, cmdData, GameManager.LocalServerId);
		}
