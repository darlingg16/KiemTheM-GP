	...
	public class YouLong_Script_Main : CopySceneEvent
	{
		...
		protected override void OnTick()
		{
			...
			if (KTGlobal.GetCurrentTimeMilis() - this.lastChallengeTicks >= YouLong.Data.KickOutIfNoChallengeFor)
			{
				...
				{
					/// Thông báo
					PlayerManager.ShowNotification(player, "Đã quá thời gian không phát sinh tỷ thí mới, tự rời phụ bản!");
				}
				/// Hủy phụ bản
				this.Dispose();
			}
		...
		public override void OnPlayerLeave(KPlayer player, GameMap toMap)
		{
			...
			this.CloseEventBroadboard(player, YouLong.Data.EventID);
		}
		...
		private void CreateBoss()
		{
			...
			/// Tạo quái
			GameManager.MonsterZoneMgr.AddDynamicMonsters(this.CopyScene.MapCode, YouLong.Data.Boss.ID, this.CopyScene.ID, 1, YouLong.Data.Boss.PosX, YouLong.Data.Boss.PosY, YouLong.Data.Boss.Name, YouLong.Data.Boss.Title, hp, this.CopyScene.Level, dir, series, YouLong.Data.Boss.AIType, -1, -1, YouLong.Data.Boss.AIScriptID, "Boss", (monster) => {
				/// Nếu có kỹ năng
		...
		{
			/// Tạo NPC
			NPCGeneralManager.AddNewNPC(this.CopyScene.MapCode, this.CopyScene.ID, YouLong.Data.NPC.ID, YouLong.Data.NPC.PosX, YouLong.Data.NPC.PosY, YouLong.Data.NPC.Name, YouLong.Data.NPC.Title, Entities.Direction.DOWN_RIGHT, -1, "", (npc) => {
				npc.Click = (player) => {
					/// Nếu không có người chơi
					if (player == null)
					{
					...
					dialog.OnSelect = (x) => {
						...
						else if (x.SelectID == -2)
						{
							...
							/// Hiện bảng nhập
							PlayerManager.ShowInputNumberBox(player, "Nhập số lượng muốn đổi.", (number) => {
								...
								/// Thêm Chiến Thư
								if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, player, YouLong.Data.RequireItem, number, 0, "YouLong_Script_Main", true, 0, false, Global.ConstGoodsEndTime, "", -1))
								{
									KNPCDialog dialogEx = new KNPCDialog();
									dialogEx.Owner = player;
		...
		public void GetRandomAward()
		{
			...
			if (this.fightStep != 2)
			{
				PlayerManager.ShowNotification(player, "Có lỗi khi nhận phần thưởng ngẫu nhiên!");
				return;
			}
		...
		public void GetAward()
		{
			...
			if (this.fightStep != 3)
			{
				PlayerManager.ShowNotification(player, "Có lỗi khi nhận phần thưởng!");
				return;
			}
			...
			{
				PlayerManager.ShowNotification(player, "Ngươi không có phần thưởng!");
				return;
			}
			...
			{
				PlayerManager.ShowNotification(player, "Túi đồ không đủ chỗ trống, hãy sắp xếp lại ít nhất " + this.currentAward.Number +  " và thử lại!");
				return;
			}
			...
			if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, player, this.currentAward.ID, this.currentAward.Number, 0, "YouLong_Script_Main", true, 0, false, Global.ConstGoodsEndTime, "", -1))
			{
				PlayerManager.ShowNotification(player, "Có lỗi khi tạo phần thưởng!");
				//return;
			}
			...
			/// Thông báo
			PlayerManager.ShowNotification(player, "Nhận thành công!");
		...
		public void ExchangeCoin()
		{
			...
			if (this.fightStep != 3)
			{
				PlayerManager.ShowNotification(player, "Có lỗi khi đổi Tiền Du Long!");
				return;
			}
			...
			if (!KTGlobal.IsHaveSpace(totalSpacesNeed, player))
			{
				PlayerManager.ShowNotification(player, "Túi đồ không đủ chỗ trống, hãy sắp xếp lại ít nhất " + this.currentAward.NumberOfCoins + " và thử lại!");
				return;
			}
			...
			/// Thực hiện thêm phần thưởng tương ứng
			if (!ItemManager.CreateItem(Global._TCPManager.TcpOutPacketPool, player, YouLong.Data.Items.YouLongCoinID, this.currentAward.NumberOfCoins, 0, "YouLong_Script_Main", true, 1, false, Global.ConstGoodsEndTime, "", -1))
			{
				PlayerManager.ShowNotification(player, "Có lỗi khi tạo Tiền Du Long!");
				//return;
			}
			...
			/// Thông báo
			PlayerManager.ShowNotification(player, "Đổi thành công!");
		...
		public void StartNewRound()
		{
			...
			if (this.fightStep != 4)
			{
				PlayerManager.ShowNotification(player, "Có lỗi khi bắt đầu vòng khiêu chiến mới!");
				return;
			}

			/// Số lượt đã tham gia trong ngày
			int challengeTimes = CopySceneEventManager.GetCopySceneTotalEnterTimesToday(player, AcitvityRecore.YouLong);
			/// Nếu đã tham gia trong ngày rồi thì thôi
			if (challengeTimes >= YouLong.Data.LimitRoundPerDay)
			{
				PlayerManager.ShowNotification(player, "Ngươi đã tham gia đủ số lượt trong ngày, không thể tham gia nữa!");
				return;
			}
			...
			{
				PlayerManager.ShowNotification(player, "Không có Chiến Thư, không thể khiêu chiến!");
				return;
			}
			...
			if (!ItemManager.RemoveItemFromBag(player, YouLong.Data.RequireItem, 1,-1, "YouLong"))
			{
				PlayerManager.ShowNotification(player, "Có lỗi khi thực hiện xóa Chiến Thư!");
				return;
			}

			/// Tăng số lượt đi trong ngày
			CopySceneEventManager.SetCopySceneTotalEnterTimesToday(player, AcitvityRecore.YouLong, challengeTimes + 1);
		...
		public void Rechallenge()
		{
			...
			{
				PlayerManager.ShowNotification(player, "Đã vượt quá số lần có thể tỷ thí lại, hãy chọn Tỷ thí mới!");
				return;
			}
		...
		public void Leave()
		{
			...
			if (this.fightStep != 4)
			{
				PlayerManager.ShowNotification(player, "Có lỗi khi thoát phụ bản!");
				return;
			}
