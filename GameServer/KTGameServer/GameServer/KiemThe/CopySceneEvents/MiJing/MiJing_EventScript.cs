...
namespace GameServer.KiemThe.CopySceneEvents.MiJingFuBen
{
	...
	public static class MiJing_EventScript
	{
		...
		public static string CheckCondition(KPlayer player)
		{
			...
			else if (CopySceneEventManager.GetCopySceneTotalEnterTimesToday(player, AcitvityRecore.MiJing) >= MiJing.LimitRoundPerDay)
			{
			...
			{
				/// Nếu đã tham gia rồi
				if (CopySceneEventManager.GetCopySceneTotalEnterTimesToday(teammate, AcitvityRecore.MiJing) >= MiJing.LimitRoundPerDay)
				{
		...
		public static void Begin(KPlayer player)
		{
			...
			foreach (KPlayer teammate in teammates)
			{
				int totalEnterTimes = CopySceneEventManager.GetCopySceneTotalEnterTimesToday(teammate, AcitvityRecore.MiJing);
				totalEnterTimes++;
				CopySceneEventManager.SetCopySceneTotalEnterTimesToday(teammate, AcitvityRecore.MiJing, totalEnterTimes);
			}
