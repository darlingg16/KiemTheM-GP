...
function LongNguThaiGia:OnOpen(scene, npc, player,otherParams)
	...
	dialog:AddSelection(3, "Du Long là gì ")
	dialog:AddSelection(4, "Hủy quan hệ đồng hành")
	dialog:AddSelection(5, "Tái Sinh đồng hành")
	dialog:AddSelection(6, "Đổi nguyên liệu đặc biệt")
	dialog:AddSelection(7, "Mở khóa trang bị đồng hành")
	dialog:AddSelection(8, "Ta chỉ tình cờ đi qua")
...
function LongNguThaiGia:OnSelection(scene, npc, player, selectionID, otherParams)
	...
	elseif selectionID == 2 then
		...
		GUI.CloseDialog(player)
	elseif selectionID == 3 or selectionID == 4 or selectionID == 5 or selectionID == 6 or selectionID == 7   then
		local dialog = GUI.CreateNPCDialog()
