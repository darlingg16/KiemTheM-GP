...
function QuanLienDauSoCap:OnOpen(scene, npc, player, otherParams)
	...
	dialog:AddText("")
	dialog:AddText("<color=orange>Lưu ý:</color> <color=yellow>Bảng xếp hạng chiến đội</color> sẽ được <color=green>cập nhật</color> sau mỗi đợt <color=yellow>bảo trì hàng ngày</color>. Cuối tháng, chỉ các chiến đội có <color=yellow>tổng số trận</color> thi đấu <color=green>từ 30 trở lên</color> mới có thể được xếp hạng nhận thưởng.")
...
function QuanLienDauSoCap:OnSelection(scene, npc, player, selectionID, otherParams)
	...
	if selectionID == 1 then
		...
		QuanLienDauSoCap:ShowNotify(npc, player, szTeamInfoText)
		return
	end
	-- ************************** --
	if selectionID == 2 then
		...
		if EventManager.TeamBattle_IsRegisterTime() == false then
			QuanLienDauSoCap:ShowNotify(npc, player, "Hiện không phải thời gian đăng ký <color=green>Võ lâm liên đấu</color>, hãy quay lại vào tháng sau!")
			return
		-- Nếu chưa có nhóm
		elseif player:HasTeam() == false then
			QuanLienDauSoCap:ShowNotify(npc, player, "Ngươi phải lập nhóm trước, sau đó mới có thể báo danh <color=green>Võ lâm liên đấu</color>.")
			return
		-- Nếu không phải nhóm trưởng
		elseif player:IsTeamLeader() == false then
			QuanLienDauSoCap:ShowNotify(npc, player, "Ngươi không phải trưởng nhóm, chỉ có trưởng nhóm mới có thể báo danh <color=green>Võ lâm liên đấu</color>.")
			return
		end
		...
		GUI.ShowInputString(player, "Nhập tên <color=yellow>chiến đội</color> tham dự <color=green>Võ lâm liên đấu</color>", function(teamName)
			...
			QuanLienDauSoCap:ShowNotify(npc, player, szCreateTeamResult)
		end)
		return
	end
	-- ************************** --
	if selectionID == 3 then
		...
		if EventManager.TeamBattle_IsBattleTimeToday() == false then
			QuanLienDauSoCap:ShowNotify(npc, player, "Hôm nay không diễn ra <color=green>Võ lâm liên đấu</color>, hãy quay lại hôm khác!")
			return
		end
	...
	if selectionID == 4 then
		...
		if player:HasTeam() == false then
			QuanLienDauSoCap:ShowNotify(npc, player, "Ngươi phải lập nhóm và tập hợp thành viên nhóm tới chỗ ta mới có thể nhận thưởng <color=green>Võ lâm liên đấu</color>.")
			return
		-- Nếu không phải nhóm trưởng
		elseif player:IsTeamLeader() == false then
			QuanLienDauSoCap:ShowNotify(npc, player, "Ngươi không phải trưởng nhóm, chỉ có trưởng nhóm mới có thể báo danh <color=green>Võ lâm liên đấu</color>.")
		...
		QuanLienDauSoCap:ShowNotify(npc, player, szGetAwardText)
		return
	end
	-- ************************** --
...
-- ======================================================= --
-- ======================================================= --
function QuanLienDauSoCap:ShowNotify(npc, player, text)
