...
function HinhBoDau:OnOpen(scene, npc, player, otherParams)
	...
	if player:GetPKValue(player) == 0 then
		...
		dialog:AddText("Ngươi sát khí quá nặng, muốn đi khỏi đây thì phải đưa ta <color=yellow>500 đồng</color>, <color=yellow>5 vạn bạc</color> để xóa sát khí. Còn không thì chịu khó ngồi đây xám hối khi nào hết sát khí ta sẽ tha!")
...
function HinhBoDau:OnSelection(scene, npc, player, selectionID, otherParams)
	...
	if selectionID == 1 then
		local dialog = GUI.CreateNPCDialog()
		dialog:AddText("Ngươi xác nhận dùng <color=yellow>500 đồng</color>, <color=yellow>5 vạn bạc</color> để tẩy toàn bộ sát khí?")
	...
	if selectionID == 11 then
		-- Nếu không đủ đồng
		if Player.CheckMoney(player, 2) < 500 then
			HinhBoDau:ShowNotify(npc, player, "Ngươi không có đủ <color=yellow>500 đồng</color>, không thể tẩy sát khí!")
			return
		-- Nếu không đủ bạc
		elseif Player.CheckMoney(player, 1) < 50000 then
			HinhBoDau:ShowNotify(npc, player, "Ngươi không có đủ <color=yellow>5 vạn bạc</color>, không thể tẩy sát khí!")
			return
		end
		
		-- Thực hiện xóa bạc và đồng
		Player.MinusMoney(player, 2, 500)
		Player.MinusMoney(player, 1, 50000)
