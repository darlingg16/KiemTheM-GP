...
function BaiHuTangChuanSongRen_ChengShi:OnSelection(scene, npc, player, selectionID, otherParams)
	...
	if selectionID == 4 then
		if player:GetLevel() >= 10 then
			if player:GetFactionID() == Global_FactionID.ShaoLin or player:GetFactionID() == Global_FactionID.TianWang then
				Player.OpenShop(npc, player, 221)
			elseif player:GetFactionID() == Global_FactionID.TangMen or player:GetFactionID() == Global_FactionID.WuDu or player:GetFactionID() == Global_FactionID.MingJiao then
				Player.OpenShop(npc, player, 222)
			elseif player:GetFactionID() == Global_FactionID.EMei or player:GetFactionID() == Global_FactionID.CuiYan or player:GetFactionID() == Global_FactionID.DaLiDuanShi then
				Player.OpenShop(npc, player, 223)
			elseif player:GetFactionID() == Global_FactionID.GaiBang or player:GetFactionID() == Global_FactionID.TianRen then
				Player.OpenShop(npc, player, 224)
			elseif player:GetFactionID() == Global_FactionID.WuDang or player:GetFactionID() == Global_FactionID.KunLun then
