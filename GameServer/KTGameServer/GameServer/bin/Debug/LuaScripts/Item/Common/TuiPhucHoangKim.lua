...
function TuiPhucHoangKim:OnUse(scene, item, player, otherParams)
	...
	if usedTimeToday == -1 then
		...
		player:AddNotification("Bạn đã dùng quá số lượng túi phúc hoàng kim trong ngày!")
		return
	end
	
	-- Tăng số lượt đã dùng
	usedTimeToday = usedTimeToday + 1
	-- Lưu lại số lượt đã dùng
	Player.SetValueOfDailyRecore(player, item:GetItemID(), usedTimeToday)
	...
	if usedTimeToday > USED_LIMIT then
		...
		Player.OpenRandomBox(player, nBoxID) 
    else 
		-- Thêm bạc khóa
