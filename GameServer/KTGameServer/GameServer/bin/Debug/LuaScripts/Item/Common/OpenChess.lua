...
function OpenChess:OnSelection(scene, item, player, selectionID, otherParams)
    ...
    if (selectionID == 1) then
        ...
        local Genre = item:GetExtParam(1);

        local DetailType = item:GetExtParam(2);

        local ParticularType = item:GetExtParam(3);

        local Level = item:GetExtParam(4);

        local Series = item:GetExtParam(5);

        local Number = item:GetExtParam(6);
